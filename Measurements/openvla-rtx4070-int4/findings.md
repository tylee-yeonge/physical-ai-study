...
그래서 이 문서에 수치가 나올 때는 대부분 `methodology.md` 의 어느 절에서 가져온 것인지 함께 적혀 있다. 숫자가 의심되면 그 절로 돌아가 조건을 확인한다.

### 0.2 절마다 성격이 다르다 — 누가 무엇을 썼는가

| 절 | 성격 | 읽는 방식 |
|---|---|---|
| §1-§3 | 2026-06 실측 당시의 판단 기록. 출처가 `Studies/Phase 4/` 의 문서로 표기돼 있다 | 결론과 근거를 함께 읽는다 |
| §4 | 이해 검증 블록. **본인이 예측하고 측정하고 오차를 설명하는 영역** | 채워진 칸은 본인 문장, `(by claude)` 표기가 있는 칸은 참고용으로 대신 작성된 것 |

§4 에서 **비어 있는 칸과 체크되지 않은 항목은 아직 하지 않은 작업**이다. 빠뜨린 것이 아니라 순서를 기다리는 것이므로, 채워진 척하지 않고 그대로 둔다. 남이 채워 주면 "본인이 설명할 수 있다" 는 이 문서의 목적 자체가 사라진다.

### 0.3 `예측 → 측정 → 오차 설명` 이 무슨 형식인가

§4 의 모든 블록이 이 형식을 쓴다. 순서가 중요하다.

```mermaid
flowchart LR
    A["예측<br/>첫 원리에서<br/>손으로 계산"] --> B["측정<br/>실제로 재기"]
    B --> C["오차 설명<br/>차이의 정체를<br/>항목별로"]
```

- **예측을 먼저 적는 이유**: 측정값을 먼저 보면 그 값에 맞는 설명을 사후에 만들게 된다. 그러면 "설명할 수 있다" 가 아니라 "결과를 아는 상태에서 말을 붙였다" 가 된다
- **오차가 남는 것이 정상**: 예측과 측정이 정확히 맞는 경우는 드물다. 중요한 것은 차이의 크기가 아니라 **그 차이를 무엇으로 설명하는가**다. 설명되지 않는 잔차가 있으면 "미확인" 으로 남기는 것이 근거 없는 설명을 붙이는 것보다 낫다
- **사다리 표**: 각 블록 끝에 있는 표가 이 형식의 요약이다. 한 줄이 하나의 `예측 / 실측 / 오차 해석` 쌍이고, 모든 줄이 채워지면 그 블록이 끝난다

### 0.4 이 문서에 자주 나오는 용어

한 번에 다 읽지 않아도 된다. 모르는 단어가 나올 때 돌아올 지점으로 둔다.

**양자화 관련**

| 용어 | 뜻 |
|---|---|
| **양자화**(quantization) | 숫자를 더 적은 비트로 표현해 메모리를 줄이는 것. 대신 정밀도를 잃는다 |
| **fp16 / bf16** | 숫자 하나를 16비트(2바이트) 로 저장하는 형식 |
| **int8 / int4** | 정수 8비트 / 4비트로 저장하는 방식 |
| **nf4**(NormalFloat 4-bit) | QLoRA 논문이 제안한 4-bit 저장 방식. 가중치 분포에 맞춘 값 배치를 쓴다 |
| **absmax** | 숫자 묶음의 절대값 최댓값. 그 값으로 나눠 범위를 맞추는 데 쓴다 |
| **scale** | 압축된 정수를 원래 크기로 되돌릴 때 곱하는 배율 |
| **double quant** | scale 자체를 다시 양자화해 메모리를 더 줄이는 옵션 |
| **packing** | 4-bit 값 두 개를 1바이트에 담아 저장하는 것. 그래서 dtype 이 uint8 로 보인다 |
| **dequant**(역양자화) | 압축 저장된 값을 다시 float 로 되돌리는 연산 |
| **quant_state** | 역양자화에 필요한 재료 (absmax 등) 를 담은 객체 |
| **compute dtype** | 저장은 4-bit 여도 실제 곱셈을 수행할 때 쓰는 자료형 |

**메모리 관련**

| 용어 | 뜻 |
|---|---|
| **`memory_allocated`** | PyTorch 가 텐서에 실제 할당한 양 |
| **`memory_reserved`** | PyTorch 가 확보해 들고 있는 양 (할당분 + 여유분) |
| **`max_memory_allocated`** | 실행 중 기록된 할당량의 최댓값 (peak) |
| **CUDA context** | GPU 를 쓰기 위해 드라이버가 잡는 기본 메모리. PyTorch 통계에 안 잡힌다 |
| **allocator** | PyTorch 의 메모리 관리자. 할당·해제가 비싸므로 큰 덩어리를 미리 받아 캐시한다 |
| **activation** | 순전파 중 각 층이 내놓은 중간 결과 텐서 |
| **KV cache** | 토큰을 하나씩 생성할 때 앞 계산을 재사용하기 위해 저장해 두는 key·value |

**추론 구조 관련**

| 용어 | 뜻 |
|---|---|
| **forward**(순전파) | 입력이 모델을 한 번 통과해 출력을 내는 과정 |
| **matmul** | 행렬 곱. 신경망 계산량의 대부분을 차지한다 |
| **weight / activation** | 학습으로 정해진 고정 파라미터 / 지금 흐르고 있는 입력 데이터 |
| **prefill** | 입력 전체(이미지 토큰 + 지시문) 를 한 번에 통과시켜 첫 출력 토큰을 만드는 국면 |
| **decode** | 그 뒤 토큰을 1개씩 이어 붙여 생성하는 국면. OpenVLA 의 action 7개가 여기 해당 |
| **GEMV** | 행렬 x 벡터 곱. 토큰 1개를 생성하는 decode 가 이 형태다 |
| **fused 커널** | 여러 단계를 하나의 GPU 커널로 묶어 실행하는 것. 중간 결과를 메모리에 쓰지 않아 빠르다 |
| **outlier** | 다른 값들보다 유별나게 큰 값. 양자화에서 정밀도를 망치는 원인이 된다 |
| **threshold** | outlier 로 볼 기준값. LLM.int8() 의 기본값은 6.0 |
| **memory-bound / compute-bound** | 소요 시간이 데이터 읽기에 지배되는 상태 / 계산에 지배되는 상태 |
| **roofline 하한** | "아무리 빨라도 이보다는 느리다" 는 이론 하한. 읽어야 하는 바이트를 대역폭으로 나눈 값 |

**평가·제어 관련**

| 용어 | 뜻 |
|---|---|
| **Bridge** | OpenVLA 논문이 성공률을 재는 실로봇 평가 환경 계열 (WidowX 팔 기반) |
| **성공률 ± 값** | 반복 평가의 평균과 그 불확실성 폭. `58.1 ± 5.1%` 는 폭을 감안해도 71% 대와 겹치지 않는다는 뜻 |
| **throughput (Hz)** | 초당 판단 횟수. `3 Hz` 면 한 번 판단에 약 333 ms |
| **퍼플렉시티**(perplexity) | 언어 모델이 정답 토큰에 부여한 확률로 만드는 점수. 낮을수록 좋다 |
| **open-loop / closed-loop** | 출력이 다음 입력에 영향을 주지 않는 구조 / 출력이 환경을 바꿔 다음 관측으로 되돌아오는 구조 |
| **quasi-static** | 환경이 거의 정지해 있어 시간 지연이 결과를 크게 바꾸지 않는 task 성격 |
| **blocking control** | 관측 -> 추론 -> 실행을 겹치지 않고 순차로 돌리는 제어 방식 |
| **action chunking** | 한 번의 정책 호출로 앞으로의 여러 스텝 action 을 한꺼번에 예측하는 방식 |
...
12GB GPU 에 7B 모델을 올리려면 양자화가 필요하고 (근거는 §4.1 Step 1), 선택지는 8-bit 와 4-bit 두 갈래였다. 이 절은 **4-bit 를 고르고 8-bit 를 버린 판단과 그 근거**다.

직관적으로는 8-bit 가 더 안전해 보인다 — 비트가 많으니 정보를 더 담고, 그러면 정확도도 높을 것 같다. 실제 공개 수치는 반대다. 그 역전의 이유를 설명하는 것이 §4.2 의 과제이고, 이 절은 **판단의 근거 수치만** 기록한다.

수치를 읽는 데 필요한 배경:

| 항목 | 뜻 |
|---|---|
| Bridge 성공률 | OpenVLA 논문이 실로봇(WidowX 팔) 에서 task 를 몇 번 성공했는지로 재는 지표 |
| `58.1 ± 5.1%` | 평균 58.1%, 불확실성 폭 ±5.1%p. 폭을 감안해도 71% 대와 겹치지 않으므로 "차이가 있다" 고 말할 수 있다 |
| `1.2 Hz` / `3 Hz` | 초당 판단 횟수. 1.2 Hz 는 한 번 판단에 약 800 ms, 3 Hz 는 약 333 ms |
| A5000 | 논문이 측정에 쓴 GPU. 우리 4070 과 다른 기기이므로 **절대값을 그대로 옮겨 쓸 수 없다** (비율로만 참고) |
| Table 2 vs §5.4 본문 | 같은 논문 안에서도 성공률·메모리는 표에, 속도는 본문에 있다. 어느 쪽을 인용했는지 적어 두면 남이 확인할 수 있다 |

- int8 Bridge 성공률 58.1 ± 5.1% — bf16 (71.3%)·int4 (71.9 ± 4.7%) 대비 유의한 하락 (OpenVLA 논문 Table 2)
- int8 추론 속도 1.2 Hz vs **int4 3 Hz** (둘 다 A5000, OpenVLA 논문 §5.4 본문) — 속도 하락이 시스템 동역학을 바꿔 성공률 하락으로 이어짐. Table 2 는 성공률·메모리만 담고 속도는 §5.4 본문에 있다 (소재 확정: §4.2 Step 5)
- 판단: 성공률·속도 모두 열위이므로 실험에서 배제. 코드 경로는 비교 실험 대비로만 보존 (week8 config)
- 미해소 항목 해소 (2026-07-28): "비트 수가 크면 덜 손실" 직관과 반대인 이유 → §4.2 Step 3·4 와 완료 판정 표. 요지는 손실이 비트 수가 아니라 forward 경로가 추가로 수행하는 작업에서 결정된다는 것

### 1.1 왜 "배제" 를 기록으로 남기는가

세 번째 항목의 "코드 경로는 보존" 이 이 절의 성격을 보여 준다. int8 을 쓰지 않기로 했지만 **쓸 수 있는 상태로 남겨 둔다** — 나중에 비교 실험이 필요해질 수 있고, 그때 다시 만드는 비용을 아끼기 위해서다.

그리고 "배제했다" 는 기록 자체가 증거다. 4-bit 를 쓴다는 결과만 있으면 "그거 하나만 해 본 것" 과 구분되지 않지만, 8-bit 를 검토하고 어떤 수치를 근거로 버렸는지가 적혀 있으면 **선택이 판단이었다는 것**이 드러난다. `Measurements/README.md` §2 가 "배제한 대안과 배제 근거" 를 필수 보존 항목으로 둔 이유다.

네 번째 항목처럼 **미해소 항목을 적어 두고 나중에 닫는 방식**도 같은 성격이다. 그때 몰랐다는 사실을 지우지 않고, 언제 무엇으로 답했는지를 함께 남긴다.
...
`methodology.md` §2 의 `300.3 ms` 를 **제어 관점의 문장으로 옮기는 절**이다. 300 ms 라는 시간 자체는 좋은지 나쁜지를 말해 주지 않는다. "이 속도로 로봇을 움직일 수 있는가" 로 바꿔야 판단이 된다.

첫 항목의 **외삽**(extrapolation)이 이 절의 방법을 보여 준다. 측정 전에 "대략 이 정도 나올 것" 을 먼저 계산해 두고, 실측과 대조하는 방식이다.

| 단계 | 내용 |
|---|---|
| 예측 근거 | 논문의 A5000 수치(3 Hz) 를 두 GPU 의 메모리 대역폭 비율(504 vs 768 GB/s) 로 환산 |
| 예측값 | 약 2-3 Hz |
| 실측 | 3.33 Hz |
| 해석 | 예측 범위보다 약 9% 빠름 — 허용 범위 안이므로 "대역폭이 지배한다" 는 전제가 크게 틀리지 않았다 |

왜 대역폭으로 환산하는가: 토큰을 하나씩 생성하는 구간은 계산량보다 **가중치를 읽어 오는 시간**이 병목이다. 그러면 속도는 대역폭에 대략 비례하므로, 다른 GPU 의 수치를 대역폭 비율로 옮길 수 있다. 이 전제 자체를 실측으로 검증하는 것이 §4.3 의 과제다.
...
### 2.1 "루프 하한 2 Hz" 가 나온 계산

네 번째 항목이 이 절에서 가장 실무적인 줄이다. 모델이 3.33 Hz 여도 **시스템 전체는 그보다 느리다.**

| 항목 | 시간 |
|---|---|
| `predict_action` 추론 | 약 300 ms |
| 카메라 캡처 + ROS2 통신 + 실행 여유 | 약 200 ms |
| 한 사이클 합계 | 약 500 ms = 2 Hz |

즉 **설계 기준은 3.33 Hz 가 아니라 2 Hz** 다. 추론 단독 수치를 시스템 성능으로 말하면 200 ms 를 잃은 계획이 되므로, 여유분을 명시해 하한을 따로 잡아 둔다.

`methodology.md` §1 이 전처리를 측정에서 제외한 것과 이 줄이 짝을 이룬다 — 제외한 몫을 어디서 다시 더했는지가 여기 적혀 있다. (전처리 자체가 무시 수준이라는 확인도 같은 항목에 있다.)

마지막 항목의 미해소는 **"2 Hz 로 충분한가" 를 아직 답하지 않았다는 뜻**이다. 답하려면 판정 기준을 먼저 정의해야 하고 (무엇에 대해 충분한지), 그 작업이 §4.4 다.
...
### 3.1 이 버그가 무엇인지 풀어 쓰면

용어부터.

| 용어 | 뜻 |
|---|---|
| **`input_ids`** | 문장을 토큰(숫자) 으로 바꾼 배열. 모델이 실제로 받는 입력 |
| **`attention_mask`** | 각 토큰이 유효한지(1) 채움용인지(0) 표시하는 같은 길이의 배열 |
| **off-by-one** | 길이나 인덱스가 딱 1 어긋나 생기는 오류. 흔하고, 조용히 틀리거나 갑자기 죽는다 |
| **토큰 29871** | Llama 계열 토크나이저에서 공백에 해당하는 토큰 번호 |
| **`**inputs`** | 딕셔너리의 모든 항목을 함수 인자로 한꺼번에 펼쳐 넘기는 파이썬 문법 |

무슨 일이 일어나는가:

1. processor 가 문장을 처리해 `input_ids` 와 `attention_mask` 를 **같은 길이**로 만들어 준다
2. `predict_action` 은 내부에서 `input_ids` 뒤에 토큰 하나(29871) 를 덧붙인다 — 모델이 학습된 프롬프트 형식을 맞추기 위해서다
3. 그런데 `attention_mask` 는 그대로다. 그래서 두 배열의 길이가 1 어긋난다
4. eager attention 은 이 두 배열을 그대로 곱해 쓰므로, 모양이 맞지 않아 그 자리에서 죽는다

**해법이 "mask 를 안 주는 것" 인 이유**: 생성 함수는 mask 가 없으면 스스로 만들어 낸다. 그때는 이미 토큰이 덧붙은 뒤의 `input_ids` 를 보고 만들므로 길이가 맞는다. 즉 내 코드가 잘못 만든 mask 를 넘기지 않는 것이 고치는 방법이다.

**참고 자료를 그대로 따라 쓰면 걸리는 함정**이기도 하다. 튜토리얼은 보통 `vla.predict_action(**inputs)` 로 쓰는데, 그러면 processor 가 만든 `attention_mask` 가 함께 넘어가 위 4번에 도달한다. 그래서 이 리포에서는 그 패턴을 전부 걷어내고 두 인자만 명시해 넘긴다.

이 절이 증거로서 갖는 의미: **수치가 아니라 판단의 기록**이다. 크래시를 만났을 때 원인을 토큰 길이까지 추적하고, 우회 방법과 그 이유를 남겼다는 것이 여기서 드러난다 (`Measurements/README.md` §2 의 "오류 유형과 원인, 선택한 해결 방법과 이유").
...
**왜 이 블록들이 있는가**: §1-§3 만 있으면 "돌려 봤고 수치를 적었다" 까지다. 검토 보고서가 지적한 것은 그 상태가 **남의 문서를 요약한 이해와 구분되지 않는다**는 점이었다. 그래서 각 수치를 첫 원리에서 손으로 예측해 보고, 실측과의 차이를 스스로 설명하는 과제를 네 개로 나눴다.

| 블록 | 답할 질문 | 새 측정 | 상태 |
|---|---|---|---|
| 1. int4 메모리 산수 | 4.38 GB 라는 값이 왜 그 값인가 | 있음 (`scripts/memory_breakdown.py`) | 완료 (2026.07) |
| 2. int8 열위 메커니즘 | 비트가 더 많은 int8 이 왜 느리고 부정확한가 | 없음 (논문 수치 + 설치본 코드 독해) | 완료 (2026.07) |
| 3. 300 ms 구간 분해 | 300 ms 중 어디가 병목인가 | 있음 (재측정과 병합) | 미착수 (2026.08 예정) |
| 4. 3.33 Hz 조건부 판정 | 이 속도로 충분한가 — 무엇에 대해 | 없음 (Block 3 결과 + 문헌) | 미착수 (2026.08 예정) |

네 블록은 독립적이지 않다. 앞 블록의 산출물이 뒤 블록의 입력이 된다.

```mermaid
flowchart LR
    B1["Block 1<br/>메모리 분해<br/>-> 파라미터 수<br/>-> 가중치 바이트"]
    B2["Block 2<br/>경로 비교<br/>-> memory-bound 가설"]
    B3["Block 3<br/>구간 분해<br/>-> 구간별 ms"]
    B4["Block 4<br/>제어 판정<br/>-> 조건부 답"]
    B1 --> B2 --> B3 --> B4
```

- Block 1 이 센 **가중치 바이트**가 Block 2 의 "int8 은 2배를 읽는다" 비교의 분모가 된다
- Block 2 가 세운 **memory-bound 가설**을 Block 3 의 decode 실측이 검증한다
- Block 3 의 **구간별 시간**이 Block 4 의 반응 시간 계산에 들어간다

**미착수 블록의 빈칸은 그대로 둔다.** 채워진 문장이 있으면 그것을 읽고 넘어가게 되고, 그러면 이 블록들의 목적 (스스로 설명할 수 있는지 확인) 이 사라진다. 각 Step 의 본문에 적혀 있는 것은 답이 아니라 **답에 도달하는 절차와 답할 질문**이다.

각 블록의 `> 골격:` 줄에는 그 블록의 작업 방식·통과 기준·예상 소요 시간이 적혀 있다. 착수 전에 그 줄을 먼저 읽는다.
...
**이 블록의 논증 구조** (5개 Step 이 한 사슬이다)

| Step | 하는 일 | 다음 Step 에 넘기는 것 |
|---|---|---|
| 1 | fp16 으로 올리면 몇 GB 인지 손계산 | 실제 파라미터 수 + "12GB 에 안 들어간다" 는 정량 근거 |
| 2 | int4 로 줄이면 몇 GB 인지 예측 (나이브 -> 정제) | 정제 예측값 |
| 3 | 예측을 `memory_allocated` 실측과 대조 | 설명되는 몫과 남는 잔차 |
| 4 | `nvidia-smi` 값과의 차이를 3항목으로 분해 | 항목별 크기 (context / 예약 / 추론 임시분) |
| 5 | 논문의 "약 7GB" 와의 간극 확인 | 소재 확정 또는 미확인 종결 |

각 Step 아래의 `->` 로 시작하는 줄이 **본인이 작성한 답**이고, 그 위 문단은 답에 도달하는 절차다. `(by claude)` 표기가 붙은 칸은 참고용으로 대신 작성된 것이므로 구분해 읽는다.

읽다가 걸리기 쉬운 두 지점을 미리 풀어 둔다.

- **왜 파라미터 수를 직접 세는가** (Step 1): 모델 이름이 `7b` 여도 실제 파라미터는 그보다 많다. 언어 모델 백본 외에 이미지를 처리하는 encoder 두 개와 그 출력을 언어 모델 입력으로 바꿔 주는 연결층이 붙어 있기 때문이다. 이름을 믿고 7.0B 로 계산하면 예측이 처음부터 어긋난다
- **왜 `sum(p.numel())` 이 과소집계인가** (Step 2 주의 문단): 4-bit 값 두 개가 1바이트에 함께 담겨 저장되므로(packing), 파라미터 개수를 세는 함수가 그 바이트를 **1개**로 센다. 실제 파라미터는 2개다. 그래서 개수를 직접 세지 않고 `methodology.md` §4 의 dtype 별 바이트 표에서 역산한다
...
| Step | 하는 일 |
|---|---|
| 1 | 틀린 직관이 깔고 있는 가정을 먼저 문장으로 꺼내 둔다 (뒤에서 판정할 대상) |
| 2 | 논문 3편의 해당 절만 정독 — 두 경로의 구조는 유도가 아니라 설계 사실이라 출처를 봐야 한다 |
| 3 | 두 경로가 forward 1회에 무엇을 하는지 표로 대조 -> 속도 차이의 메커니즘 |
| 4 | 정확도(성공률) 차이의 인과 경로 -> 가정 2 판정 |
| 5 | "fp16 대비 int4 는 빠른가" 방향 예측 후 논문 수치와 대조 |

**측정 없이 어떻게 검증하는가**: 이 블록에는 새 측정이 없다. `예측 -> 측정 -> 오차` 형식의 "측정" 자리에 **논문의 공개 수치(A5000 기준) 와 설치된 라이브러리의 실제 소스코드**가 들어간다. int8 을 직접 재려면 모델을 int8 로 다시 올려야 하는데, fp16 은 4070 에 아예 안 올라가고(Block 1 Step 1) int8 은 이 실험의 배제 대상이므로 재지 않는다. 그래서 결론의 어미가 "실측했다" 가 아니라 "논문 수치와 코드 구조에서 도출했다" 로 쓰여 있다 — **근거의 종류를 문장에 반영하는 것**이 이 블록의 규칙이다.

읽다가 걸리기 쉬운 지점:

- **"경로"**(path) 라는 말이 계속 나온다: Linear 층 하나가 forward 1회에 실제로 실행하는 절차 전체를 뜻한다. int8 과 int4 는 저장 비트만 다른 것이 아니라 **서로 다른 함수를 실행한다** — 그 차이가 이 블록의 답이다
- **정독 항목이 4개(a-d) 인 이유**: (a)(b)(c) 는 int8 쪽 경로의 근거 (LLM.int8() 논문), (d) 는 int4 쪽 경로의 근거 (QLoRA 논문) 다. 그리고 (e) 에서 설치본 코드를 열어 두 논문의 주장이 실제 구현과 맞는지 확인한다. **논문 -> 코드 순서**가 이 Step 의 설계다
...
- [x] **Step 5 — "fp16 대비 int4 속도" 방향 예측 → Table 2 대조**
  ...
  **선행 개념 (예측을 쓰기 전에 확인)**
  ...
  - 기록 (by claude — 본인 도출 아님. 자가 검증 10문항에서 이 절의 내용이 판정 대상이다)
    - **예측: 근사 동등** (int4 가 fp16 대비 크게 빠르지 않다). 두 효과의 **적용 범위가 다르다**는 것이 근거다.
      1) **(a) 가중치 트래픽 1/4 은 decode 구간에만 온전히 적용된다.** `predict_action` 1회는 vision encoder (SigLIP + DINOv2) 1회 + prefill 1회 (visual 256 + prompt, 약 280 토큰) + decode 7회로 구성된다. 앞의 둘은 행렬×행렬이라 compute 비중이 크고 (선행 개념의 판정 기준), 바이트를 줄여도 속도로 전환되지 않는다. 즉 이득이 붙는 자리가 전체의 일부다
      2) **(b) dequant 비용은 전 구간에 붙는다.** decode 는 `gemv_4bit` 로 커널 내부 fused 지만, prefill 은 `MatMul4Bit` 에서 `dequantize_4bit` 로 fp16 텐서를 만든 뒤 `F.linear` 를 호출한다 ((e) int4 2) — 후자는 fp16 로 풀린 가중치를 쓰고 다시 읽는 왕복이 생기므로, 바이트를 줄인 이득을 그 구간에서 되돌린다
      3) 따라서 roofline 약 4배는 **decode 단독의 상한**이고, `predict_action` 전체로 오면 (b) 가 (a) 를 상당 부분 상쇄한다. 예측 문장은 "int4 가 빠르다" 가 아니라 **"메모리를 4배 줄이면서 속도는 지킨다"** 다
    - **논문 수치 대조**
      - A5000: **int4 3 Hz**, int8 1.2 Hz — 둘 다 OpenVLA §5.4 **본문**이다
      - **A5000 의 bf16 Hz 는 논문에 없다** → int4/bf16 비율을 직접 산출할 수 없다. **미확인으로 종결** (Block 1 Step 5 와 같은 방식)
      - 정성 근거로 대체: Figure 5 캡션 — "bfloat16 과 int4 양자화 **모두** 높은 throughput 을 달성하며, 특히 Ada Lovelace (RTX 4090, H100) 에서 그렇다". 두 정밀도를 한 문장에 묶은 것이 근사 동등의 진술이다
      - 참조점: §3.5 — bf16 은 15 GB 를 쓰며 RTX 4090 1장에서 약 6 Hz. A5000 (768 GB/s) 로 대역폭 비례 외삽하면 4-5 Hz 대가 되지만, 아키텍처가 다른 기기 간 외삽이라 근거가 약하다 — **논문 수치가 아니라 본인 외삽임을 명시**
      - **소재 정정**: 속도 수치는 §5.4 본문이고 **Table 2 는 성공률·메모리만** 담는다. Table 2 캡션의 "4-bit 가 bfloat16 의 성능을 맞춘다" 에서 성능은 **성공률**이지 속도가 아니다 — 아래 완료 판정 표의 근거 셀을 이에 맞춰 정정했다
    - **오차 설명**
      - 예측 (근사 동등) 과 논문 (int4 3 Hz + Figure 5 의 정성 동등) 은 **방향이 일치**한다. 다만 비율로 검증하지 못했으므로 "일치 확인" 이라고 단정하지 않는다
      - roofline 상한 약 4배와 실제 근사 동등 사이 간극의 후보: vision encoder·prefill 의 compute 비중 / prefill 경로 dequant 의 fp16 텐서 왕복 / Ampere 의 bf16 텐서코어가 이미 빨라 줄일 여지가 적다는 점 / 커널 효율
      - **이 배분은 지금 수치로 확정되지 않는다.** `predict_action` 300 ms 안에서 vision encoder·prefill·decode 가 각각 몇 ms 인지가 나와야 하고, 그것이 **Block 3 Step 3** 이다. decode 실측 (토큰당 ms) 을 roofline 하한과 대조하면 (a) 가 실제로 어디까지 유효했는지 판정된다
      - 어미: int4 3 Hz·int8 1.2 Hz 는 논문 실측이지만, 근사 동등이라는 예측 근거는 본인 실측이 아니라 설치본 코드 구조와 논문 진술에서 도출한 판단이다 (fp16 은 4070 에서 로드 불가 — Block 1 Step 1)

완료 판정 표 — 각 행이 본인 문장으로 채워지면 Block 2 완료 (아래는 by claude 작성):

| 질문 | 답변 요지 | 대조 근거 |
|---|---|---|
| int8 이 int4 보다 느린 메커니즘 | 비트 수가 아니라 경로 구조의 차이다. decode 는 memory-bound 인데 int8 은 가중치를 int4 의 2배 읽고, 그 위에 activation absmax 양자화·threshold 스캔·outlier 열 분해·출력 dequant 가 매 forward 얹힌다. NF4 경로에는 이 네 가지가 없고 weight dequant 만 있다. 6.7B 급에서 outlier 가 전 층에 나타나므로 이 비용은 예외가 아니라 상수로 추정된다 | Step 3 표 + OpenVLA §5.4 본문 (int8 1.2 Hz vs int4 3 Hz, A5000) |
| int8 성공률 58.1% 의 인과 경로 | 두 지표가 다른 것을 잰다 — LLM.int8() 의 무손실 주장은 퍼플렉시티·제로샷 정확도 (출력이 다음 입력을 바꾸지 않는 open-loop 정적 지표) 에 대한 것이고, 58.1% 는 closed-loop 에피소드 성공률이다. int8 이 1.2 Hz 로 느려지면 관측에서 행동까지의 지연이 커져 궤적 보정 간격이 벌어지고, 구간 내 오차가 더 오래 방치돼 실패가 누적된다. 속도 요인과 표현 손실 요인의 기여도는 논문에 분리 측정된 바 없어 미확인 | OpenVLA §5.4 + LLM.int8() 무손실 주장 |
| fp16 대비 int4 속도 방향 | 근사 동등. int4 의 이득 (가중치 바이트 1/4) 은 decode 7토큰에만 온전히 적용되고 vision encoder·prefill 은 compute 비중이 커서, 전 구간에 붙는 dequant 비용 (특히 prefill 경로의 fp16 텐서 왕복) 이 이를 상당 부분 상쇄한다. **A5000 의 bf16 Hz 가 논문에 없어 비율 검증은 미확인 종결** | §5.4 본문 (int4 3 Hz) + Figure 5 캡션 (bf16·int4 모두 high throughput). Table 2 는 성공률·메모리만 담는다 |
| §1 미해소 항목 한 줄 답 | 양자화의 손실은 비트 수가 아니라 forward 경로가 무엇을 추가로 수행하는지에서 결정되고, 로봇에서는 그 추가 비용이 속도로 나타나 closed-loop 성공률을 깎는다 — 그래서 int8 이 int4 보다 비트가 많아도 열위다 | 위 세 행의 종합 |
...
**이 블록의 논증 구조** (미착수 — 아래는 무엇을 할 것인지의 지도이고, 답은 착수 시 본인이 채운다)

| Step | 하는 일 | 산출물 |
|---|---|---|
| 1 | 300 ms 가 구간별로 어떻게 나뉠지 **측정 전에** 손계산 | 사다리 표의 "예측" 열 |
| 2 | `predict_action` 내부를 고치지 않고 구간 경계를 재는 방법 설계 | `scripts/latency_breakdown.py` |
| 3 | 실측 후 예측 대비 오차를 구간별로 설명 | 사다리 표 완성 + `raw/`·`summary.csv` |
| 4 | 이미지 해상도를 절반으로 줄이면 어디가 얼마나 변할지 예측 -> 측정 | 개입 실험 표 |
| 5 | 측정 조건 자체의 타당성 검증 3종 (warm-up 횟수, sync 필요성, 재현성) | 조건 근거 수치 |

**왜 구간을 나누는가**: `300 ms` 는 한 덩어리 숫자라서 그것으로 할 수 있는 판단이 없다. 어디가 얼마를 쓰는지 알면 (1) 최적화할 지점이 정해지고, (2) 다른 장비로 옮길 때의 예측이 가능해지고, (3) Block 2 에서 세운 "decode 는 메모리 읽기가 병목" 가설을 실제로 검증할 수 있다. 세 번째가 이 블록이 Block 2 의 뒤에 오는 이유다.

**Step 4 가 왜 붙어 있는가**: 구간 분해만으로는 "이 구간이 무엇에 비례하는가" 를 알 수 없다. 입력 해상도를 절반으로 줄이는 **개입**을 넣고 어느 구간이 줄고 어느 구간이 그대로인지 보면, 각 구간의 비용이 무엇에 의존하는지가 드러난다. 관찰만으로는 상관까지, 개입을 넣으면 의존 구조까지 볼 수 있다.

**측정이 실패할 수도 있다**: Step 4 는 "112x112 이미지가 vision encoder 를 통과하지 않을 수 있다" 는 조건 확인부터 시작한다. 통과하지 못하면 예측만 남기고 "구조 제약으로 측정 불가" 로 종결한다 — 억지로 통과시키는 것보다 **못 한 이유를 적는 것**이 기록으로서 정직하다 (Block 1 Step 5 의 미확인 종결과 같은 방식).
...
| Step | 하는 일 | 산출물 |
|---|---|---|
| 1 | 논문 2-3편의 해당 절만 정독 (action chunking, blocking control, 저수준 루프 주기) | 용어 두 쌍의 정의 |
| 2 | 로봇 제어가 몇 개 층으로 되어 있고 3.33 Hz 가 어느 층의 숫자인지 표로 정리 | 제어 계층 표 |
| 3 | "충분한가" 를 답할 수 있는 형태의 정량 기준을 정의하고 task 유형별로 판정 | 수치 예 2개 (정적 / 동적) |
| 4 | action chunking 이 이 판정을 어떻게 바꾸는지 (그리고 못 바꾸는 것은 무엇인지) | 실행 주파수 vs 반응 주파수 구분 |

**이 블록의 성격**: 새 측정이 없다. §2 가 남긴 미해소 항목 — "3.33 Hz 로 충분한가" — 를 **답할 수 있는 질문으로 바꾸는 작업**이다. 그대로는 답할 수 없는 질문이기 때문이다. "충분" 은 무엇에 대해 충분한지를 정하지 않으면 판정 기준이 없고, 기준이 없으면 어떤 답도 근거를 갖지 못한다.

**왜 이것이 Block 3 뒤에 오는가**: 판정에는 "관측에서 행동까지 몇 ms 가 걸리는가" 가 필요하고, 그 값을 구간 분해(Block 3) 가 준다. 추론 300 ms 만으로 판정하면 카메라·통신·실행 몫이 빠진 답이 된다 (§2.1 의 500 ms 계산이 그 이유다).

**답의 형태가 "예 / 아니오" 가 아니다**: 같은 3.33 Hz 가 정지한 물체를 집는 task 에서는 충분하고 움직이는 물체를 따라가는 task 에서는 부족할 수 있다. 그래서 **task 유형별 조건부 답**이 산출물이고, 각 유형마다 수치 예를 하나씩 만들어 판정한다.
