...
gantt
    ...
    Stage 1 (SO-101 본 빌드, v2 선행) :h1, 2027-01, 2M
...
| 2027.01-02 | Stage 1 | **Hardware-Arm Stage 1 (SO-101 본 빌드, 스파이크로 디리스크)** + URDF + Sim 디지털 트윈 (v2 선행) | v2 하드웨어 기반 |
...
> **코어/엣지 구조** (2026-08-05 확정): **코어** = 로봇 시스템 SW 통합 (ROS2 아키텍처·배포·운영) — 어디서나 팔리는 층, 회사가 VLA 를 안 해도 지원 가능. **엣지** = Physical AI/VLA 준비 (OpenVLA 배포·LoRA 파이프라인·sim eval) — 다른 ROS 5년차와 구별하는 상단 레이어. "VLA 통합 엔지니어" 직함은 시장에 그대로 존재하지 않으므로 지원 시 각 회사 JD 언어 (Robotics Software Engineer / System Integration Engineer / AI Deployment Engineer 등) 로 번역한다. 상세: [`docs/superpowers/specs/2026-08-05-career-positioning-vla-edge-design.md`](./docs/superpowers/specs/2026-08-05-career-positioning-vla-edge-design.md)
