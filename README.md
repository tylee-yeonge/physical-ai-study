...
> [goal] **목표**: AMR ROS 양산 SW + Physical AI 통합 → **VLA/Foundation Model 통합 엔지니어**
...
[O] **딥러닝 학습 (Phase 3~5)**: **Python** (PyTorch, HuggingFace)

[O] **딥러닝 배포 (Phase 3, 7)**: **PC TensorRT + ROS2 노드** (Jetson 30+ FPS 는 Phase 7 이후 옵션)

[O] **로봇 통합 (Phase 6, 7, Hardware-Arm)**: **ROS2 + URDF + Isaac Sim**
...
> [note] **Jetson 실기 배포는 Phase 7 이후 옵션** — 전체 학습 (Phase 2~7) 완료 후 시간 여유 보고 결정
...
> [tip] 결과물 없는 학습 금지. 각 주제별 짧은 노트 또는 mini-demo 1개.
...
> [tip] **보너스**: Dynamixel 채택 = **Dynamixel 제조사이자 휴머노이드 양산 상장사 (2순위 C)** 지원 시 직접 매칭 (모터 제조사 = 회사 자체).
...
```
현재: AMR ROS Application 개발자 (양산 AMR, 9년차)
      |
      v
...
6. **AMR 실무 경험이 차별점, Foundation Model 통합이 본체**: 9년 SW 경력 (자동차 R&D 보조 + 펌웨어 + AMR ROS) + 자작 팔 통합이 최대 무기

---

## [tip] 흔들리지 않기 위한 3원칙

1. **"관심 있다" ≠ "공부한다"** — NeRF / 3D Gaussian Splatting / World Model 등은 "흥미로움" 폴더에만 저장. 현재 Phase 와 직접 관련 없으면 학습 시간 쓰지 않기.
2. **결과물 없는 학습은 학습이 아님** — 블로그 글, GitHub repo, 발표 자료 중 하나로 결과물 정의. 책·강의만 본 시간은 진척으로 카운트하지 않기.
3. **분기별 리뷰** — 3개월마다 "원래 계획 vs 실제 진행" 비교. 방향은 흔들지 않되, 페이스 조정은 허용.

---

## [appendix] 부록 A: Phase ↔ 디렉토리 매핑
...
> [*] **#4 가 본 일정의 핵심**:
> - *"Sim only 산출물은 박사도 만든다. Sim + 자작 실 팔이면 본인만 만든다"*
> - *"VLA latency 200ms / 안전 메커니즘 / 양산 비용"* 의 직접 증거 (양산 SW 엔지니어의 차별화 메시지)
> - 본인 9년 경력 (자동차 R&D 보조 + 펌웨어 + AMR ROS) 이 한 점에 수렴

---

## [appendix] 부록 C: Phase 7 이후 옵션 — Jetson 실기 배포
...
## [appendix] 부록 D: 6개월 분기 재평가 메타-규칙
...
> [note] **경로**: 기하학 기초 → **VLA 논문** → **자작 팔** → **Foundation Model** → **Isaac Sim 디지털 트윈** → **Real-to-Sim-to-Real (#4 결정타)** → **이직**
