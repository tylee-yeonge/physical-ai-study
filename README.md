...
> **보조 서사** (선택, 관점 1줄): *"팔 중심 공개 VLA 의 perception+action 구조를 분석해, 내 5년 도메인인 이동 로봇 플랫폼으로의 이식 관점에서 본다."* — 자작 팔을 그 자체 목적이 아니라 이동 로봇 이식 인사이트를 얻는 도구로 위치. 이건 하드웨어를 이동으로 바꾸자는 약속이 아니라 분석 lens 다. 하드웨어 베팅 자체(팔 유지 vs 이동 도입)는 2026.11 재평가 입력(부록 D)으로 따로 둔다.
...
| 시점 | 재평가 항목 |
|---|---|
| **2026.11** | 하드웨어 스파이크 결과 (파이프라인 검증·일정 재산정) / Phase 4 (VLA 논문) 진행률 / 시장 신호 probe 반응 (JD 격차·커피챗·실지원 1건) / **콘텐츠 반응** (조회·인바운드·댓글·star) / OpenVLA 후속 모델 등장 여부 / **팔 vs 이동 하드웨어 베팅 재검토** (공개 VLA 는 팔 조작 중심인데 내 5년 해자는 AMR 이동 로봇 — 자작 팔을 "이동 플랫폼 이식 도구" 로 프레이밍 유지할지 vs 이동 검증을 일부 끌어들일지를 스파이크 결과·시간 예산과 함께 판단. ⚠ "이동 피벗" 결론이면 Hardware-Arm / Phase 6·7 / 산출물 #2.5·#4 가 연쇄로 바뀌는 구조 변경이므로 그 비용을 전제로 본다) |
