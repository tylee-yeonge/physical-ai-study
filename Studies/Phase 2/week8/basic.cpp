...
void OpticalFlowBasic::visualizeFlow(const cv::Mat& flow,
    ...
    for (int y = 0; y < flow.rows; y += step) {
        for (int x = 0; x < flow.cols; x += step) {
            ...
            // 화살표 그리기
            cv::arrowedLine(output, start, end,
...
void OpticalFlowBasic::visualizeSparseFlow(const cv::Mat& img,
    ...
    for (size_t i = 0; i < prev_pts.size(); i++) {
        if (status[i]) {
            // 궤적 그리기
            cv::line(output, prev_pts[i], curr_pts[i],
                    cv::Scalar(0, 255, 0), 2);
            
            // 현재 위치
            cv::circle(output, curr_pts[i], 5,
                      cv::Scalar(0, 0, 255), -1);
        }
    }
}
...
void OpticalFlowBasic::demoFeatureTracking(const std::string& video_path) {
    ...
    for (int i = 0; i < num_frames; i++) {
        ...
        // 움직이는 원
        cv::circle(frame, cv::Point(100 + i*50, 240), 30,
                  cv::Scalar(255), -1);
        
        // 고정된 패턴
        for (int j = 0; j < 5; j++) {
            cv::rectangle(frame,
                        cv::Point(400 + j*40, 100 + j*60),
                        cv::Point(430 + j*40, 130 + j*60),
                        cv::Scalar(200), -1);
        }
    ...
    for (size_t i = 1; i < frames.size(); i++) {
        ...
        double success_rate = lucasKanade(frames[i-1], frames[i],
                                         prev_pts, curr_pts, status);
        
        std::cout << "Frame " << i << ": "
                  << "추적 성공률 " << std::fixed << std::setprecision(1)
...
void OpticalFlowBasic::demoPipeline() {
    ...
    std::cout << "   평균 이동: " << std::fixed << std::setprecision(2)
              << avg_motion << " 픽셀\n" << std::endl;
