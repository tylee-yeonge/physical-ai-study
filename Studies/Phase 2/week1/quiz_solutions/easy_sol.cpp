...
void problem3_fov_calculation()
{
    ...
    std::cout << std::endl;
    std::cout << "   [Perception 시스템에서의 의미]" << std::endl;
    std::cout << "   - 광각(FOV>90°): 한 프레임에 객체가 많이 보여 검출 누락이 적지만, 렌즈 왜곡이 큼" << std::endl;
    std::cout << "   - 망원(FOV<30°): 먼 물체를 자세히 볼 수 있지만, 시야가 좁음" << std::endl;
}
...
void problem4_back_projection()
{
    ...
    std::cout << "   Multi-view 삼각측량은 '두 카메라의 역투영 광선이 만나는 점'을 구하는 것입니다." << std::endl;
}
...
void problem5_visibility_check()
{
    ...
    std::cout << std::endl;
    std::cout << "   [Perception 후처리에서의 활용]" << std::endl;
    std::cout << "   3D Detection 결과의 박스 코너, BEV frustum culling 등에서 동일한 가시성 체크가 사용됨." << std::endl;
    std::cout << "   보이는 점/코너만 시각화 / 평가에 사용 → 계산 효율 + 결과 신뢰도 향상" << std::endl;
}
