...
void problem4_decompose_essential()
{
    ...
    //   특이값은 이론적으로 [σ, σ, 0] 형태여야 함

    // TODO 2: W 행렬 정의
    //   W = [0 -1 0; 1 0 0; 0 0 1]  (90도 Z축 회전)

    // TODO 3: 4가지 해 계산
    //   R1 = U * W^T * Vt, R2 = U * W * Vt
    //   t1 = +U.col(2),    t2 = -U.col(2)
    //   ★ det(R) < 0 이면 R = -R (반사 행렬 보정)

    // TODO 4: Cheirality Check
    //   각 (R, t) 해로 3D 점 삼각측량 후:
    //   - 카메라 1: Z_cam1 > 0 확인
    //   - 카메라 2: X_cam2 = R*X + t 에서 Z_cam2 > 0 확인
    //   - 가장 높은 Z > 0 비율의 해 = 올바른 해
...
void problem5_failure_cases()
{
    ...
    //   cv::SVD::compute(E_small, w, u, vt)
    //   w의 값이 매우 작으면 → 수치적으로 불안정
    //   노이즈(~1e-3)가 신호(~1e-6)보다 클 수 있음
