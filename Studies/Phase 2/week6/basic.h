...
public:
     ...
     * @return 성공 여부
     */
    static bool triangulatePoint(const cv::Point2f& pt1,
     ...
     * @return 재투영 오차 (픽셀)
     */
    static double reprojectionError(const cv::Point3f& point3d,
     ...
     * @return 깊이 (m)
     */
    static double disparityToDepth(double disparity,
                                   double baseline,
                                   double focal_length);
     ...
     */
    static bool isInFrontOfCamera(const cv::Point3f& point3d,
                                  const cv::Mat& R,
                                  const cv::Mat& t);
