...
`./my_basic` 실행 시 Step 5 재투영 오차의 **평균 / 최대 / RMS** 픽셀값을 콘솔에 출력하면 Week 4 마무리.


- 합성 데이터 기준 평균 < 0.01 px (사실상 0)
- KITTI / nuScenes 실데이터 기준 평균 < 5 px


이 기준 미달이면 Phase 3 디버깅(특히 카메라 좌표/투영)에서 두 배로 막힐 가능성 — 다음 주로 미루는 게 낫다.
