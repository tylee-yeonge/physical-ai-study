...
void problem2_essential_matrix()
{
    ...
    // R 후보: U · W · V^T  vs  U · W^T · V^T
    //
    //   E = U · Σ · V^T 에서 U와 V^T는 각각 카메라2/카메라1 좌표계를 "표준 위치"로
    //   정렬하는 역할이고, 그 사이에서 실제 회전을 결정하는 것이 W이다.
    //
    //   [t]×(skew-symmetric)의 성질 [t]× = -[t]×^T 때문에,
    //   +90도(W)와 -90도(W^T) 회전이 모두 유효한 해가 된다.
    //   R1과 R2는 서로 약 180도 차이 — 카메라가 앞을 보는 경우 vs 뒤를 보는 경우.
    cv::Mat R1 = u_e * W * vt_e;
    cv::Mat R2 = u_e * W.t() * vt_e;

    // t 후보: ±U의 3번째 열
    //
    //   [t]×는 rank 2이고, t 자체가 [t]×의 null space이다 ([t]× · t = t × t = 0).
    //   SVD에서 특이값 0에 대응하는 U의 열이 이 null space 방향이므로:
    //     t의 방향 = U의 3번째 열
    //
    //   E와 -E가 같은 SVD 결과를 줄 수 있어 t의 부호를 결정할 수 없다.
    //   따라서 ±를 모두 후보로 둔다. (이동 방향이 정반대인 두 경우)
    ...
    // [전체 흐름]
    //   4가지 (R,t) 후보를 하나씩 시도한다.
    //   각 후보마다 모든 매칭 점 쌍을 삼각측량해서 3D 점을 복원하고,
    //   그 점이 두 카메라 모두의 "앞"에 있는지 확인한다.
    //   앞에 있는 점이 가장 많은 후보가 정답이다.
    for (const auto& [Rc, tc] : candidates)
    {
        int positive_depth_count = 0;
        for (int i = 0; i < N; i++)
        {
            // p1, p2: 정규화된 이미지 좌표 (K를 제거한 상태)
            // 이미 K^{-1}을 적용했으므로, 카메라 투영 행렬은 단위 행렬로 취급할 수 있다.
            // [x, y, 1]^T 형태 = 카메라 원점에서 해당 픽셀 방향으로의 광선 벡터
            cv::Mat p1 = (cv::Mat_<double>(3, 1) << norm_pts1[i].x, norm_pts1[i].y, 1.0);
            cv::Mat p2 = (cv::Mat_<double>(3, 1) << norm_pts2[i].x, norm_pts2[i].y, 1.0);

            // ── 삼각측량 (Triangulation) ──
            //
            //   두 카메라에서 같은 점을 본 광선의 교차점 = 3D 점 X = [X, Y, Z]^T
            //
            //   [카메라1] 원점이 O1(=원점), 투영 P1 = [I | 0]
            //
            //     투영 행렬 P는 3D 점 X를 2D 이미지 좌표로 변환하는 3×4 행렬이다.
            //     카메라1을 기준 카메라(원점)로 잡으면:
            //       - 회전 = 없음 → I (3×3 단위행렬)
            //       - 이동 = 없음 → 0 (3×1 영벡터)
            //     따라서 P1 = [I | 0] (3×4 행렬)
            //
            //     투영 관계: λ · p1 = [I | 0] · [X,Y,Z,1]^T = I · X + 0 = X
            //       여기서 p1 = [x1, y1, 1]^T (정규화된 이미지 좌표)
            //
            //       λ(람다)란? — 스케일 팩터(scale factor), 물리적으로는 깊이(depth) = Z값
            //       카메라는 3D→2D 투영 시 깊이 정보를 잃어버린다.
            //       p1은 카메라 원점에서 픽셀 방향으로의 방향 벡터일 뿐,
            //       그 방향 위 어디에 실제 3D 점이 있는지는 알 수 없다.
            //       λ가 "그 방향으로 얼마나 멀리 있는가"를 알려주는 값이다.
            //         예) λ=2 → Z=2 거리에 있는 점, λ=10 → Z=10 거리에 있는 점
            //       카메라 하나로는 λ를 알 수 없다 (광선 위 무한히 많은 점이 가능).
            //       → 그래서 두 번째 카메라를 써서 두 광선의 교차점을 구하는 것이 삼각측량이다.
            //
            //     성분별로 풀면:
            //       λ · x1 = X
            //       λ · y1 = Y
            //       λ · 1  = Z  →  λ = Z
            //
            //     λ = Z를 대입하면: x1 = X/Z, y1 = Y/Z  (핀홀 카메라 투영 공식)
            //
            //     거꾸로 보면 X = λ · p1 이므로,
            //     "3D 점 X는 카메라 원점에서 p1 방향으로 λ(=Z)만큼 떨어진 점"
            //     → 이것이 카메라 원점에서 픽셀 방향으로 쏜 광선(ray) 개념이다.
            //     이를 정리하면:
            //       X - x1·Z = 0  ← 행1
            //       Y - y1·Z = 0  ← 행2
            //
            //   [카메라2] 원점이 O2, 투영 P2 = [R | t]
            //
            //     카메라2는 카메라1(원점) 기준으로 회전 R, 이동 t만큼 움직인 카메라이다.
            //     카메라1에서는 자기가 원점이라 변환 없이 P1 = [I | 0]으로 단순했지만,
            //     카메라2는 위치가 다르므로 좌표 변환이 필요하다 → P2 = [R | t]
            //
            //     투영 관계: μ · p2 = [R | t] · [X,Y,Z,1]^T = R · X + t
            //       여기서 p2 = [x2, y2, 1]^T (카메라2에서 본 정규화된 이미지 좌표)
            //       R · X + t = 3D 점 X를 카메라2 좌표계로 변환한 것
            //
            //       μ(뮤)란? — 카메라2에서의 스케일 팩터, 즉 카메라2 좌표계에서의 깊이(Z값)
            //       카메라1의 λ가 카메라1에서의 깊이였듯이,
            //       μ는 카메라2에서 바라본 깊이이다. 카메라 위치가 다르므로 일반적으로 λ ≠ μ.
            //
            //     성분별로 풀면:
            //       μ · x2 = R행1·X + tx
            //       μ · y2 = R행2·X + ty
            //       μ · 1  = R행3·X + tz  →  μ = R행3·X + tz
            //
            //     μ로 나누면:
            //       x2 = (R행1·X + tx) / (R행3·X + tz)
            //       y2 = (R행2·X + ty) / (R행3·X + tz)
            //     분모를 곱해서 정리하면 (교차곱 전개):
            //       (R행1 - x2·R행3) · X = -(tx - x2·tz)  ← 행3
            //       (R행2 - y2·R행3) · X = -(ty - y2·tz)  ← 행4
            //
            //   이렇게 4개 방정식 (미지수 3개: X,Y,Z) → 과결정 시스템 A·X = b

            cv::Mat A_tri(4, 3, CV_64F);   // 4×3 계수 행렬
            cv::Mat b_tri(4, 1, CV_64F);   // 4×1 우변 벡터

            // 카메라1 방정식 (행1~2):
            //   행1: [1, 0, -x1] · [X,Y,Z]^T = 0   → X - x1·Z = 0
            //   행2: [0, 1, -y1] · [X,Y,Z]^T = 0   → Y - y1·Z = 0
            ...
            // 카메라2 방정식 (행3~4):
            //   행3: (R행1 - x2·R행3) · X = -(tx - x2·tz)
            //   행4: (R행2 - y2·R행3) · X = -(ty - y2·tz)
            //   분모(R행3·X + tz)를 양변에 곱해서 분수를 제거한 형태이다.
            ...
            // A·X = b 를 SVD 기반 최소제곱으로 풀기
            // 4개 방정식, 3개 미지수 → 과결정(overdetermined) 시스템
            //   과결정 = 조건(방정식)이 미지수보다 많아서 모두 동시에 만족하는 정확한 해가 없는 상태
            //   (노이즈로 인해 두 광선이 정확히 한 점에서 만나지 않고 살짝 엇갈림)
            //   → 정확한 해 대신 ||A·X - b||² 를 최소화하는 최소제곱해를 구한다.
            //
            // cv::DECOMP_SVD: SVD(특이값 분해)를 사용해서 풀라는 옵션
            //   일반적인 풀이 X = A⁻¹·b는 A가 정사각 행렬일 때만 가능한데,
            //   여기서 A는 4×3 (비정사각)이라 역행렬이 존재하지 않는다.
            //   SVD는 비정사각·과결정 시스템에서도 최소제곱해(pseudo-inverse)를 구할 수 있어
            //   이런 경우에 필수적인 선택이다.
            cv::Mat X;
            cv::solve(A_tri, b_tri, X, cv::DECOMP_SVD);

            // ── Cheirality 검증: 깊이(Z) 부호 확인 ──
            //
            //   X = [X, Y, Z]^T 는 카메라1 좌표계 기준의 3D 점이다.
            //   z1 = Z  → 카메라1 기준 깊이. 양수면 카메라1 앞에 있다.
            double z1 = X.at<double>(2);

            //   카메라2 좌표계에서 보려면: X_cam2 = R · X + t
            //   z2 = X_cam2의 Z성분 → 카메라2 기준 깊이. 양수면 카메라2 앞에 있다.
            cv::Mat X_cam2 = Rc * X + tc;
            double z2 = X_cam2.at<double>(2);

            // 두 카메라 모두에서 양의 깊이 → 물체가 두 카메라 앞에 있음 → 유효한 점
            if (z1 > 0 && z2 > 0)
                positive_depth_count++;
        }

        // 유효한 점이 가장 많은 (R,t) 조합을 최종 답으로 선택
