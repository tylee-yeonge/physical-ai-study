...
- **여는 곳 (순서대로)**: [ros2_driver_setup.md](ros2_driver_setup.md) 헤더 → §0 → §1 (선행 설치 → 빌드 → 드라이버의 사실) → §2 → §4.1 (팔 없이) → §4.2 (실제 팔) → §4.3 (Foxglove 화면)
...
- **하는 일**
  ...
  6. ③ 화면으로 확인 — 이 컨테이너에는 화면이 없어 RViz 를 띄울 수 없다. 화면은 Foxglove 다 ([ros2_driver_setup.md](ros2_driver_setup.md) §4.3 — 브리지 띄우기 · 접속 주소 · 3D 패널 설정 · 터미널 세 개 배치). mock 에서 헬퍼로 보낸 명령이 3D 패널의 팔에 반영되고, 실제 팔에서 +0.05 rad 명령이 눈앞의 팔과 화면의 팔을 같은 방향으로 움직이면 ③ 완료다. URDF 자체는 `check_urdf` 로 화면 없이도 확인된다. 오프셋 맞추기 ([URDF_guide.md](URDF_guide.md) §0 의 3번) 는 W4 로 남긴다
- **끝나면 남는 것**: 빌드되는 드라이버, 실제 팔의 6축 `/joint_states`, 한 관절을 의도한 만큼 움직인 기록, Foxglove 에서 팔이 명령대로 움직이는 화면
