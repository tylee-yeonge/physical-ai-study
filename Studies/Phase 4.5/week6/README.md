...
B 의 "구간 겹침" 을 풀어 두면 이렇다. zero-shot 구간이 0-5%, fine-tuned 구간이 3-15% 라면 두 범위가 3-5% 에서 겹친다. 즉 **참값이 둘 다 4% 일 가능성이 배제되지 않는다.** 관측된 차이가 실제 차이인지 우연인지 이 N 으로는 가릴 수 없다는 뜻이고, 그것은 "차이가 없다" 와도 다르다.
