...
이득이 학습 시간에만 있는 것이 아니다. **`grad_accumulation_steps` 가 체크포인트 저장의 반복 횟수를 그대로 결정한다** (아래 실습 4 참고). 16 에서 8 로 낮추면 저장 이벤트 1회 비용도 절반이 된다 — 저장 한 번에 10분이 드는 상황에서 이것은 학습 단축과 같은 무게를 갖는다.
...
- `--save_steps` 를 정하는 법: 회수 시 잃는 진행을 제한하려면 자주 저장해야 하지만, **저장 한 번의 비용이 크다.** base 를 다시 읽어 머지하고 15GB 를 쓰며, 저장 조건이 배치 루프 안에 있어 **grad accumulation 횟수만큼 같은 스텝이 중복 저장된다.** grad accumulation 16 이면 한 번의 저장이 16회(약 10분) 로 늘어난다. 총 스텝의 1/5 이 출발점이지만, 실습 3 에서 잰 저장 1회 소요 시간을 곱해 보고 학습 시간의 10-20% 를 넘지 않는 값으로 올린다.
- **저장 중에는 진행률이 멈춘 것처럼 보인다.** `Saving Model Checkpoint for Step N` 과 `Loading checkpoint shards` 가 반복 출력되는 동안 tqdm 이 갱신되지 않는다. 죽은 것과 구별하는 방법은 반복 횟수를 세는 것이다 — 이 값이 `grad_accumulation_steps` 에 도달하면 저장이 끝난다. tmux 새 창(`Ctrl+b` 다음 `c`)에서 확인하고 `Ctrl+b` 다음 `0` 으로 돌아온다.
...
- 이 구간에서 **절대 `Ctrl+C` 를 누르지 않는다.** 그때까지의 진행이 사라진다. 화살표 키도 누르지 않는다 — tqdm 이 갱신되지 않는 상태라 입력이 `^[[A` 형태로 화면에 그대로 찍힌다 (학습에는 영향이 없지만 화면을 읽기 어려워진다).
