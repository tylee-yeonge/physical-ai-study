...
| **쿼터니언**(quaternion) | 회전을 4개 숫자로 나타내는 표현. 짐벌락(오일러각에서 두 회전축이 겹쳐 자유도 하나를 잃는 현상)이 없어 시뮬레이터 내부에서 자주 쓴다 |
| **DoF**(degrees of freedom) | 자유도. "7-DoF action" 은 7개 숫자로 이루어진 action 벡터라는 뜻 |
...
숫자로 보면 이렇다. 스텝 t 의 TCP 위치가 `(0.40, 0.10, 0.20)` 이고 스텝 t+1 이 `(0.41, 0.10, 0.19)` 이면, 그 스텝의 위치 delta 는 뺄셈으로 `(+0.01, 0, -0.01)` — "x 로 1cm, z 로 -1cm" 다. 단 **회전은 뺄셈이 아니다.** 회전의 delta 는 "지금 자세에서 다음 자세로 가려면 어떤 회전을 얹어야 하는가" 라는 상대 회전을 계산해야 하고, 그 결과를 RPY 로 표현해 저장한다. 회전 각도를 성분별로 빼는 실수는 각도가 작을 때 표가 안 나므로 (week0 §6 과 같은 함정) round-trip 검증으로만 잡힌다.
