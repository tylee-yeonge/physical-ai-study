...
| 4 | action 변환 계약 확정 | `PRACTICE.md` 4 | 단위 / 범위 / 프레임 / gripper 부호 4항목 + 제어 주기 |
...
이번 주는 sim·모델·측정 세 영역의 용어가 한꺼번에 나온다. 여기서 "이게 무슨 역할의 물건인가" 만 잡고 시작한다.


| 용어 | 뜻 |
|---|---|
| **zero-shot** | 추가 학습 없이 사전학습된 모델을 그대로 새 환경에 투입하는 것 |
| **baseline** | 개선 작업 전의 기준 성적. 이후의 모든 결과는 이 값과의 차이로 읽는다 |
| **하네스**(harness) | 모델을 둘러싼 측정 장치 일체 — env 생성, 이미지 추출, action 변환, 성공 집계 루프 |
| **closed-loop** | 관측 -> 행동 -> 다음 관측이 꼬리를 물고 이어지는 실행 방식. 한 번 틀리면 다음 입력도 어긋난다 |
| **VLA**(Vision-Language-Action) | 카메라 이미지와 언어 지시를 받아 로봇 action 을 내놓는 모델. OpenVLA 가 그 하나다 |
| **episode** | 시작부터 성공/실패/시간 초과까지의 한 판 |
| **step** | episode 안의 한 순간. "관측을 주고 action 을 받아 물리를 한 번 진행" 이 1 step |
| **EEF**(end-effector) | 로봇 팔 끝의 그리퍼(손) 부분 |
| **delta** | 절대 위치가 아니라 "지금 위치에서 얼마나 움직일지" 의 증분 |
| **정규화**(normalization) | 서로 다른 스케일의 값을 공통 대역(`[-1, 1]` 등)으로 환산하는 것 |
| **`unnorm_key`** | 모델의 정규화된 출력을 어느 로봇의 물리 단위로 되돌릴지 고르는 스위치 (§5) |
| **헤드리스**(headless) | 모니터 창 없이 렌더 결과를 숫자 배열로만 받는 실행 방식 |
| **Vulkan** | GPU 렌더링 표준 API. ManiSkill 의 렌더러(SAPIEN)가 이것으로 그린다 |
| **venv** | 파이썬 패키지를 프로젝트별로 격리하는 가상 환경 |
| **seed** | 무작위 초기 배치를 재현 가능하게 만드는 숫자. 같은 seed 면 같은 배치가 나온다 |
| **step cap**(`max_episode_steps`) | 한 episode 에 허용된 최대 step 수. 넘으면 실패로 잘린다 |
| **action repeat** | 정책 1회 결정을 같은 값으로 sim 여러 step 에 반복 적용해 제어 주기를 맞추는 기법 (§4) |
| **motion planning / scripted** | 학습 없이 기하 계산·대본으로 팔을 움직이는 기성 해법. 상한 대조의 도구 (§8) |
| **도메인 갭**(domain gap) | 모델이 학습한 데이터와 투입된 환경 사이의 차이. 성능 하락의 "정상적" 원인 |
...
비유하면 이렇다. 고장 여부를 모르는 저울로 몸무게를 재면 숫자가 몇이 나와도 믿을 수 없다. 그래서 먼저 무게를 아는 표준 추를 올려 **저울 자체**를 검증한다. 이번 주의 상한 대조·공개 수치 대조 (§8) 가 그 표준 추다 — 추를 올려 본 적 없는 저울의 눈금은 데이터가 아니다.
...
### 3. ManiSkill 과 OpenVLA — 역할 분담과 루프 구조


두 이름이 나란히 나오지만 비교 가능한 두 대안이 아니다. **범주가 다른 한 쌍**이다. ManiSkill 은 로봇과 물체가 존재하고 물리가 작동하는 **세계**이고, OpenVLA 는 그 세계를 보고 다음 동작을 결정하는 **뇌**다. 한쪽이 내놓는 것을 다른 쪽이 받아야 루프가 닫힌다.


| 항목 | ManiSkill | OpenVLA |
|---|---|---|
| 정체 | 시뮬레이터 + 벤치마크 task 모음 | 사전학습된 VLA 정책 모델 |
| 입력 | action 벡터 | 카메라 이미지 + 영어 명령어 |
| 출력 | 관측, 보상, 성공 판정, 렌더 이미지 | action 벡터 |
| 학습된 것 | 없다. 물리 규칙과 task 정의는 코드로 고정 | Open-X Embodiment 데이터로 사전학습 |
| 무작위성 | 초기 배치 (seed 로 재현) | 없다 (`do_sample=False` 로 결정적) |
| 실행 비용 | 가볍다 (GPU 물리·렌더) | 무겁다 (7B 모델 추론) |
| 이번 주 역할 | 측정 무대 — **성공률의 정의를 소유한다** | 측정 대상 — 성공률의 원인 |


OpenVLA 쪽 구조는 Phase 4 week4 에서 다뤘다 ([`Phase 4/week4`](../../Phase%204/week4/README.md)) — Llama 7B 언어 모델에 DINOv2·SigLIP 결합 비전 인코더를 붙인 형태다. 이번 주에 중요한 것은 그 구조가 **action 을 내놓는 방식**이다. 별도의 회귀 출력층이 없고 언어 모델 어휘의 일부 토큰을 action bin 으로 재활용한다. 토큰은 정해진 개수의 이산 구간만 표현할 수 있으므로, 그 값을 실제 물리량으로 되돌리는 단계 (§5 의 `unnorm_key`) 가 구조적으로 반드시 필요하다.


"토큰을 action bin 으로 재활용한다" 를 풀면 이렇다. 언어 모델은 원래 "다음 단어(토큰)를 어휘 목록에서 고르는 기계" 다. OpenVLA 는 그 어휘에서 거의 안 쓰이는 토큰 256개를 골라 새 용도를 붙였다 — `[-1, 1]` 구간을 256칸으로 나누고, "k번째 토큰 = k번째 칸의 값" 이라고 정한 것이다. 모델이 토큰 7개를 차례로 뱉으면 그것이 7차원 action 벡터가 된다.


| 단계 | 값의 형태 | 예 |
|---|---|---|
| 모델이 뱉는다 | 토큰 7개 | bin 137, bin 92, ... |
| 토큰 -> 숫자 | 정규화된 값 (`[-1, 1]` 안) | 0.07, -0.28, ... |
| 역정규화 (`unnorm_key`) | 물리량 | 0.004 m, -0.03 rad, ... |


256칸이 표현의 전부이므로 그보다 고운 값은 낼 수 없고 (이산화), 물리량으로 돌아가려면 반드시 마지막 줄의 역정규화를 거쳐야 한다.


ManiSkill 은 **gymnasium** 규약을 따른다 — 강화학습 환경들의 표준 인터페이스로, `env.reset()` 으로 한 판을 시작하고 `env.step(action)` 으로 한 스텝 진행한다는 약속이다. 규약이 같으므로 어떤 환경이든 같은 루프 코드로 돌릴 수 있다. 두 쪽을 이어 붙인 한 episode 는 아래 흐름이다.


```mermaid
flowchart LR
    subgraph MS["ManiSkill (세계)"]
        CAM["카메라 관측<br/>추출"]
        STEP["step(action)"]
        DONE{"success<br/>또는<br/>step cap"}
    end
    subgraph VLA["OpenVLA (뇌)"]
        VIS["비전 인코더"]
        LLM["언어 모델"]
        TOK["action 토큰<br/>-> 정규화값"]
        UN["역정규화<br/>unnorm_key"]
    end
    ADP["변환 레이어<br/>단위<br/>범위<br/>프레임<br/>부호"]
    CAM -->|"이미지"| VIS
    VIS --> LLM --> TOK --> UN
    UN -->|"물리량"| ADP
    ADP -->|"정규화 action"| STEP
    STEP --> DONE
    DONE -->|"아니오"| CAM
    DONE -->|"예"| END["episode 종료"]
```


가운데 `변환 레이어` 가 이번 주에 새로 만드는 것이다 (§4, 실습 4). 두 블록은 이 레이어 없이는 물리적으로 연결되지 않는다.
...
`env.step(action)` 은 `(obs, reward, terminated, truncated, info)` 를 돌려주고, task 성공 여부는 `info` 안에 실린다. 다섯 값 중 `terminated` 는 "task 가 끝났다"(성공 판정 등), `truncated` 는 "시간이 다 되어 잘렸다"(step cap 초과) — 서로 다른 종류의 종료다. 이 구분은 week2 의 RLDS 필드 (`is_terminal` / `is_last`) 에서 같은 모양으로 다시 나온다. **키 이름과 성공 판정의 정확한 정의는 설치된 버전에서 직접 확인한다** (실습 3) — 버전마다 다르므로 문서를 믿지 않는다.


### 4. action 변환 계약 — 이번 주의 성패를 가르는 지점


OpenVLA 는 7개 숫자를 내놓는다. ManiSkill 은 7개 숫자를 받는다. 개수가 같다는 것은 **아무것도 보장하지 않는다.** 플러그 모양이 맞아 콘센트에 꽂히는 것과, 전압·극성이 맞아 기기가 사는 것이 별개인 것과 같다 — 아래 4가지가 전압에 해당하고, 전부 맞아야 팔이 의도한 대로 움직인다.
...
**4항목 밖에 하나가 더 있다 — 제어 주기.** 위 4개는 "7개 숫자를 어떻게 읽을지"의 문제인데, 그와 별개로 **한 스텝이 현실의 몇 초인지**가 양쪽에서 다르다. 정책이 학습된 데이터가 5 Hz (한 스텝 0.2초) 로 기록됐고 sim 이 20 Hz (한 스텝 0.05초) 로 돌면, 모델이 "0.2초 동안 3cm 가라"고 낸 값을 0.05초에 실행하게 되어 팔이 학습 시보다 4배 빠르게 움직인다. 단위·부호가 다 맞아도 동작은 어긋난다.


수단은 **action repeat** 다. 정책을 한 번 호출하고 같은 action 을 sim 에 여러 step 연속으로 넣어 실효 주기를 맞춘다. 배수는 두 주기의 비율이다 (20 Hz / 5 Hz = 4).


이 선택이 step 예산을 바꾼다. 정책 결정 1회가 env step 여러 개를 소비하므로, 정책에 같은 횟수의 판단 기회를 주려면 환경의 `max_episode_steps` 를 그만큼 늘려야 한다. 그리고 **이 값들은 baseline 정의의 일부**다 — 실습 5 의 상한 대조와 실습 6 의 측정이 같은 예산을 써야 두 결과를 비교할 수 있고, 이후 fine-tuned 측정도 같은 값을 써야 before/after 가 성립한다.
...
풀어서 보면 이렇다. 같은 "한 스텝" 이라도 큰 산업용 팔은 2cm 를 움직이고 작은 데스크톱 팔은 5mm 를 움직인다. 이 데이터를 그대로 섞어 학습하면 크기가 뒤죽박죽이라, 로봇(데이터셋)별로 분포를 조사해 각자 `[-1, 1]` 로 환산한 뒤 학습한다 — 모델이 배우는 것은 "그 로봇 기준의 상대적 크기" 다. 출력을 실제 팔에 보내려면 환산을 되돌려야 하는데 환산표가 로봇마다 다르므로, **어느 표로 되돌릴지**를 골라야 한다. 그 스위치가 `unnorm_key` 고, 이번 주에 쓰는 `bridge_orig` 는 "WidowX 팔로 수집된 Bridge 데이터의 환산표" 라는 뜻이다.
...
왜 작은 델타에서 비슷해지는가. 회전각이 작으면 (대략 10도 이하) 축-각 벡터의 세 성분과 RPY 세 각이 수치적으로 거의 같아진다 — 두 표현의 차이가 각도의 제곱 항부터 나타나기 때문이다. 한 스텝의 델타 회전은 늘 이 "작은" 영역에 있으므로, 표현을 바꿔 끼워도 팔은 얼추 비슷하게 돈다. 다만 스텝마다 조금씩 어긋난 회전이 누적되고, 큰 회전이 필요한 순간에 크게 틀린다 — 그래서 눈이 아니라 검증 절차 (week1 의 round-trip) 로 잡아야 한다.
...
"정합 처리" 가 없다는 것의 구체적 내용 하나는 **카메라 규약**이다. 위 표의 앞 두 줄은 실제 로봇 데이터의 시점·화각을 sim 에서 맞춰 재현한 환경들이다. 반면 기성 sim task 의 카메라는 그 task 를 보기 좋게 담도록 배치된 것이라, 정책이 학습에서 본 시점과 아무 관계가 없다. 팔의 모양이 다른 것 (§5) 과 별개로, **정책 입장에서는 화면 자체가 처음 보는 것**이다. 이 규약을 맞출 수 있는지가 week1 의 task 확정에서 갈리는 지점이다.
...
**상한과 함께 하한도 재야 한다.** task 의 초기 배치를 무작위화하면, 물체가 이미 목표 조건을 만족한 상태로 시작하는 episode 가 섞일 수 있다. 그런 episode 는 정책이 아무것도 하지 않아도 성공으로 집계된다. 그래서 **무행동 정책**(전 차원 0 인 action)으로 같은 seed 목록을 돌려 "공짜로 얻는 성공"이 몇 개인지 먼저 확정한다. 이 값을 모르면 zero-shot 성공률이 1/20 로 나왔을 때 그것이 정책의 성과인지 초기 배치 운인지 구분할 수 없고, before/after 비교에서도 같은 혼동이 반복된다. 하한에 해당하는 seed 번호를 기록해 두는 것이 최소 조치다.
...
신뢰구간이 낯설면 동전으로 생각한다. 진짜 확률이 50% 인 동전도 20번 던지면 앞면이 6번이나 14번 나오는 일이 드물지 않다. 측정된 성공률도 마찬가지로, N=20 에서는 진짜 값 주위로 크게 흔들린다 — ±22%p 가 그 흔들림의 폭이다. 그래서 이 숫자 하나로 미세한 우열을 가리는 주장은 할 수 없고, before/after 처럼 큰 차이를 보는 용도로 쓴다. 구간을 제대로 다루는 방법은 week5 에서 배운다.
...
3. **변환 계약 4항목** (단위 / 범위 / 프레임·회전 / gripper 부호) 을 출처와 함께 표로 확정하고, 여기에 **제어 주기**를 맞추는 action repeat 까지 정한다.
