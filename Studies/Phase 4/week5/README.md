...
> [goal] **이번 주 목표**: OpenX-Embodiment 의 22 embodiments 와 970K episodes 의 구조를 파악하고, fine-tuning (특히 LoRA) 의 실제 흐름을 코드 수준에서 이해한다.
> [time] **예상 시간**: 10시간 (논문 Sec 4 재정독 3h + datasets 라이브러리 실습 4h + 노트 3h)
> [tip] **핵심 질문**: "내 자작 6DOF 팔 (Phase 7) 의 데이터는 OpenX-Embodiment 의 어떤 embodiment 와 가장 가까운가? 그 데이터의 trajectory format 은 어떻게 생겼는가?"
...
> [tip] 본 로드맵 자작 팔 (Dynamixel XM430 6DOF) 와 가장 가까운 것은 **WidowX 250** (6-DoF Dynamixel 기반). Phase 7 의 LoRA fine-tune 시 WidowX subset 의 trajectory 가 가장 유용한 학습 prior.
...
> [tip] batch_size=1 + gradient_accumulation=8 이 본 로드맵 RTX 4070 환경의 표준.
...
[O] 이전: [Week 4 - OpenVLA 1회독 + Architecture](../week4/README.md)
