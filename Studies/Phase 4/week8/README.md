...
> [goal] **이번 주 목표**: week 6 의 단발 inference 코드를 **안정적인 inference 서버** 형태로 재구성. 실제 이미지 (mock 이 아닌) 로 정확한 input format 검증. ROS2 통합 (week 10~) 의 사전 준비.
> [time] **예상 시간**: 10시간 (안정화 4h + image pipeline 3h + 에러 처리 3h)
> [tip] **핵심 질문**: "OpenVLA inference 가 100회 연속으로 한 번도 fail 없이 동작하는가? OOM / shape mismatch / NaN 등 어떤 edge case 가 있는가?"
...
> [tip] ELP Stereo 는 left + right 가 한 frame 에 합쳐져서 나옴 (1280x480). 한쪽만 잘라 사용.
...
[O] 이전: [Week 7 - OpenVLA 블로그 1편](../week7/README.md)
