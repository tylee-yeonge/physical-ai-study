...
- **4070 (우분투 PC)**: 반납하지 않는다. **2026.09 자택 이전 확정**이라 휴직 기간에도 물리 접근이 유지되고, GPU 종속 작업의 8월 마감 근거는 소멸했다. 원격 운용 전환 점검(자동 부팅·원격 접속 경로·원격 전원 수단)은 출장지 사용 대비로 남기고 필요할 때 수행한다 — 상세: remediation plan 실행 보드 #12.
...
- **정식 8월 초 체크포인트 안건** (입력: 7월 실가용 실적): (1) Phase 4.5 배치 재확인 (Section 0 2026.08 / Sections 1-3 2026.09-11), (2) 하드웨어 스파이크 실행 시기 — 9월 절충안 vs 원안 10월. SO-101 키트 구매 시점이 여기에 연동되고, 구매 전 판매자 확인 4항목을 처리한다, (3) 순서 6 정독 잠정치 갱신, (4) git filter-repo 실행 여부 확정 — 레포 비공개 유지·산출물 repo 분리로 불요 예상, 산출물 repo 신원 설정 확인으로 대체 (진행 원칙 항목).

  **4070 PC 자택 이전은 2026.09 로 확정됐다** (안건에서 해소). GPU 종속 작업의 8월 마감 근거가 소멸했으므로 8월 배치는 물리 접근이 아니라 가용 시간만으로 판단한다.
