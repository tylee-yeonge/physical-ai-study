...
- **순서 6-7 잠정치**: 순서 6 정독 20-30h(2h 블록에 적합, wall-clock 약 3-4주) — 실적 기록이 없어 잠정치를 유지하고 `week2` 실소요로 확정한다. 배치는 Phase 4.5 의 실행 대기 시간(week3 LoRA 학습, week5 eval N회)을 흡수처로 쓴다. 순서 7 은 v2 이관.
- **Phase 4.5 배치**: Section 0 (week0) 은 2026-08-03 완료. Section 1 (week1-2) 도 8월에 끝났고, week3-6 을 8월 안에 단일 트랙으로 완주시킨다 — 실적 페이스가 추정을 앞질러 원안(Sections 1-3 을 2026.09-11)보다 빠르다. 조기 경보: 8월 3주차까지 week3 의 20스텝 probe 가 안 돌면 8월 완주 전제를 버리고 9-11월 배치를 다시 짠다. Section 0 을 앞에 둔 이유는 v1 과 독립적이면서 리스크가 가장 커서, 2026.11 분기 재평가를 가설이 아니라 실물 위에서 하기 위함이다.
...
- **8월 체크포인트 결론** (2026-08-12): (1) Phase 4.5 는 위 배치대로 8월 완주를 목표로 한다, (2) **하드웨어 스파이크는 2026.10 실행 / 키트 구매만 2026.09** — 9월은 휴직 적응기와 자택 이전이 겹쳐 2-3주 하드웨어 블록에 부적합하고, Phase 4.5 를 8월에 끝내면 10월의 트랙 충돌(Section 2-3 병행)이 사라진다. 10월 실행이면 재평가 #1(2026.11)의 입력 타이밍도 맞는다. 구매 전 판매자 확인 4항목을 9월 초에 문의한다, (3) 정독 잠정치는 20-30h 유지 + `week2` 실측으로 확정, (4) git filter-repo 는 실행하지 않는다 (진행 원칙 항목).
