...
| 5 | `week11`-`week12` 마무리 | dry-run 결과 정리 (latency/throughput 표) + 8월 체크포인트 | 6월 5주차(목표 6/30) | 4h | 대기 |
...
- 8월 초 체크포인트: v1 기술 코어(순서 4) 확보 직후 8-12월 예산·배치를 확정한다 (순서 5 의 마지막 체크 항목). 입력: 7월 실가용 실적(사후 집계), v1 진행 결과. 1순위 검토 안건: Phase 4.5(v1.5) 의 9-10월 조기 진입. 순서 6 정독 잠정치(20-30h)도 여기서 갱신한다.
...
- **v1.5 조기 진입 결정 — 전면 조기 진입(9월 착수) 보류**. 사유: (1) 2h 제약으로 페이스 3-4배 감소, (2) v1 에서 sim 을 빼면서 Phase 4.5 가 sim 구축 + zero-shot baseline 측정까지 흡수해 6-8주 → 7-10주로 커짐, (3) `Studies/Phase 4.5/` 학습 자료가 week README 스캐폴드뿐이라(PRACTICE/quiz 미작성) 자료 작성 시간이 추가됨, (4) Colab A100 가용성 변수. 대안: **Phase 4.5 Section 0(리스크 스파이크 — Colab LoRA 1사이클 실측 + ManiSkill sim 구축 + zero-shot 성공률 baseline)만 9-10월 선행**한다. v1 과 독립적이고 리스크가 가장 크므로 분기 재평가(2026.11)를 가설이 아닌 실물 위에서 하기 위함. 본 학습(Section 1-3)은 11-12월 유지하고, Section 0 실측 결과로 일정을 재확정한다.
