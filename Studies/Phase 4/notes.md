...
- 6월 미달 시 절삭 순서 (실가용이 5h 추세로 56h에 미달할 때): (1) 버퍼 소진 (6월 잔여 + 7월 첫 주) → (2) 성공률 측정 N 축소: 베이스라인 20 → 하한 10 → (3) 측정 항목 축소 (latency 분포는 유지, 부가 지표 절삭) → (4) 그래도 미달이면 순서 4 week12(영상 준비분)를 8월(순서 7)로 이월 — v1 공개 일정 불변. v1 산출물 정의(루트 README 부록 B)를 건드리는 절삭은 별도 의사결정 — 발동 시 부록 B 갱신 + 2026.11 분기 재평가 #1 안건 등재가 전제.
...
- 7월 가용분 처리: 7월에 시간이 나면 순서 6(정독+블로그)을 선행한다. 안 나면 8월로 미뤄도 v1 공개(8-9월) 일정은 영향받지 않는다 — 순서 6은 본래 8-9월 작업이다.
- 작업-시간 매칭: 연속 블록(통합·디버깅·dry-run)은 6월 종일 가용일에 우선 배치. 순서 5 측정 반복·quiz는 짧은 블록에서도 가능하므로 7월 단시간 가용일의 흡수처로 둔다.
- 8월 초 체크포인트: v1 기술 코어(순서 5) 확보 직후 8-12월 예산·배치를 확정한다 (순서 5 의 마지막 체크 항목). 입력: 7월 실가용 실적(사후 집계), v1 진행 결과. 1순위 검토 안건: Phase 4.5(v1.5) 의 9-10월 조기 진입.
...
- git author 재작성(filter-repo) 결정 기한: 순서 6 진입 전(8월 초 체크포인트)까지. 공개·fork 이후 불가하므로 그 전에 실행 여부를 확정한다.
