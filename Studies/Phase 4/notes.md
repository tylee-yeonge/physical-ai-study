...
- 8월 초 체크포인트: v1 기술 코어(순서 4) 확보 직후 8-12월 예산·배치를 확정한다 (순서 5 의 마지막 체크 항목). 입력: 7월 실가용 실적(사후 집계), v1 진행 결과. Phase 4.5 배치(Section 0 2026.08 / Sections 1-3 2026.09-11)는 아래 노트에 산정돼 있고 이 자리에서 재확인한다 — 안건 목록은 아래 "정식 8월 초 체크포인트 안건" bullet 을 따른다. 순서 6 정독 잠정치(20-30h)도 여기서 갱신한다.
...
정식 8월 초 체크포인트 전에 미리 산정한 결론이다. 7월 실가용 실적을 입력으로 그 자리에서 재확인한다.

- **진척 실적**: 6월(하루 5-8h)에 순서 1-4(약 56h) 완주, 순서 4(v1 기술 코어) 6/29 확보. 6/30 부터 가용이 평일 2h 로 줄어 페이스는 주 8-10h.
- **8-12월 예산**: 평일 2h x 약 4-5일 = 주 약 8-10h, 월 약 30-35h(출장/피로 변수로 보수적). 남은 v1 = 순서 5(4h) + 순서 6 정독(20-30h) = 약 24-34h.
- **순서 6-7 잠정치**: 순서 6 정독 20-30h(2h 블록에 적합, wall-clock 약 3-4주). 순서 7 은 v2 이관.
- **Phase 4.5 배치**: Section 0 은 2026.08, Sections 1-3 은 2026.09-11. 전면 조기 진입은 하지 않는다 — 사유: (1) 2h 제약으로 페이스 3-4배 감소, (2) v1 에서 sim 을 뺀 만큼 Phase 4.5 가 sim 구축 + zero-shot baseline 까지 흡수해 7-10주 규모, (3) `Studies/Phase 4.5/` 자료가 week README 스캐폴드뿐이라 작성 시간 추가, (4) 클라우드 GPU 가용성·비용 변수. Section 0 을 앞에 두는 이유는 v1 과 독립적이면서 리스크가 가장 커서, 2026.11 분기 재평가를 가설이 아니라 실물 위에서 하기 위함이다. Section 0 의 Docker 화·RunPod 이관이 Sections 1-3 의 로컬 GPU 비의존 경로를 만든다.
- **4070 (우분투 PC)**: 반납하지 않는다. 휴직 개시(2026.09) 후에는 PC 장애 시 현장 방문 전까지 복구 불가라 GPU 물리 접근 작업은 8월 마감을 권장으로 유지하고, 8월 중 원격 운용 전환 점검(자동 부팅·원격 접속 경로·원격 전원 수단)을 수행한다 — 상세: remediation plan 실행 보드 #12. 자택 이전 조율 중이며 성사되면 이 마감이 해소된다.
- **자작 팔**: **SO-101 (SO-ARM101) 리더-팔로워**. 근거는 채용 관점에서 읽히는 것이 모터 브랜드(타깃사 액추에이터 정합)가 아니라 LeRobot/HF 파이프라인 경험이고, 그 생태계의 현행 표준이 SO-101 이라는 판단이다. 국내 조달 리드타임 3일이라 발주 시점 제약이 없고, 3D 프린팅 부품이 키트에 포함돼 출력 경로를 따로 확보할 필요가 없다. 스파이크는 조립 전 드라이버 선검증이고, Stage 2 확장 수단은 2026.11 재평가 안건이다. 상세: [Hardware-Arm.md](../../Roadmap/Hardware-Arm.md).
- **정식 8월 초 체크포인트 안건** (입력: 7월 실가용 실적): (1) Phase 4.5 배치 재확인 (Section 0 2026.08 / Sections 1-3 2026.09-11), (2) 하드웨어 스파이크 실행 시기 — 9월 절충안 vs 원안 10월 (8월은 GPU 종속 작업 과밀로 제외). SO-101 키트 구매 시점이 여기에 연동되고, 구매 전 판매자 확인 4항목을 처리한다, (3) 순서 6 정독 잠정치 갱신, (4) git filter-repo 실행 여부 확정 — 레포 비공개 유지·산출물 repo 분리로 불요 예상, 산출물 repo 신원 설정 확인으로 대체 (진행 원칙 항목), (5) **4070 PC 자택 이전 여부와 시점** — 성사 시 GPU 종속 작업의 8월 마감 근거가 소멸하므로 8월 배치를 재작성해야 한다. **나머지 안건보다 먼저 확정한다.**
