...
| 6 | `week1`,`week2`,`week4`,`week5`,`week6` (정독만) | RT-2/OpenVLA 정독 — 아키텍처 이해 (블로그 작성 `week3`/`week7` 은 v2 로 이관) | 8-9월 | 20-30h 잠정 (8월 초 체크포인트에서 갱신) | 진행 중 |
...
- 순서 1-5 (6-7월) 합계 약 60h = 계획선 (순서 5 가 성공률 측정 제외로 22h→4h 로 축소됨). 최대 가용(96h)과의 차이는 버퍼 — 막히는 구간(실습·통합·dry-run 디버깅)에 흡수한다.
- 6월 미달 시 절삭 순서 (실가용이 5h 추세로 미달할 때): (1) 버퍼 소진 (6월 잔여 + 7월 첫 주) → (2) dry-run 부가 지표 축소 (latency 분포는 유지, 그 외 절삭). (성공률 측정 N 축소는 sim 을 v1 에서 빼면서 더 이상 v1 절삭 대상이 아니다 — Phase 4.5 사안.) v1 산출물 정의(루트 README 부록 B)를 건드리는 절삭은 별도 의사결정 — 발동 시 부록 B 갱신 + 2026.11 분기 재평가 #1 안건 등재가 전제. (영상·패키징·공개는 v1 에서 이미 제외되어 v2 로 이관됐으므로 절삭 대상이 아니다.)
- 가용 시간 (2026-06-29 갱신): 6월(하루 5-8h)에 순서 1-4 완주, v1 기술 코어(순서 4) 6/29 확보. **6/30 부터 평일 퇴근 후 약 2h(주 약 8-10h, 월 약 30-35h)로 축소** — 주말은 가족 우선(계획 비포함). 종일 연속 블록은 6월로 종료됐고, 큰 통합·디버깅을 6월에 끝낸 것이 다행이다. 남은 v1(순서 5 4h + 순서 6 정독 20-30h)은 2h 블록에 적합한 작업이라 페이스 축소 영향이 작다.
...
- 8월 초 체크포인트: v1 기술 코어(순서 4) 확보 직후 8-12월 예산·배치를 확정한다 (순서 5 의 마지막 체크 항목). 입력: 7월 실가용 실적(사후 집계), v1 진행 결과. 조기 수행분(아래 노트)에서 Phase 4.5 전면 조기 진입 보류·Section 0 의 2026.08 전진이 기결 — 정식 안건 목록은 아래 "정식 8월 초 체크포인트 안건" bullet 을 따른다. 순서 6 정독 잠정치(20-30h)도 여기서 갱신한다.
...
- git author 재작성(filter-repo) 결정 기한: 8월 초 체크포인트까지 실행 여부를 확정한다. 이 레포 비공개 유지 + 공개 산출물 별도 repo 분리 결정(2026-07-20)으로 이 레포의 author 재작성은 사실상 불요 예상 — 산출물 repo 를 처음부터 올바른 신원으로 생성하는 것으로 대체하고, 체크포인트에서 최종 확정한다.
...
> **이관 안내 (2026-06)**: v1 에서 sim 을 제외하면서, 아래 sim 선정·성공 task 정의·dry-run criteria 노트는 v1 에서 직접 쓰이지 않고 Phase 4.5(v1.5, sim 결합)로 이관됐다. 분석 자체는 4.5 의 sim 환경/task 구성에 그대로 재사용하므로 삭제하지 않고 보존한다. embodiment/action/아키텍처 이해(순서 2)는 sim 과 무관하게 v1 에 유효하다.
...
- [x] `week11/README.md` + `week11/PRACTICE.md` 실습 1-3 (실 inference 통합 / 빌드+실행 / 1분 dry-run + 통계) — 입력은 USB 카메라 + ros2 bag 재생 (sim 연결은 v1 에서 제외, Phase 4.5 로 이관) + quiz
- [x] `week12/README.md` + `week12/PRACTICE.md` 실습 1-2 (Rerun 기본 / rerun_logger 노드) — **레포 결과 기록용 시각화로만 선택 수행** (스크린샷/짧은 gif 1장이면 충분). 1분 영상 제작(실습 3-5)은 v1 에서 제외, v2 로 이관 + quiz

## 순서 5 — dry-run 결과 정리 + 체크포인트 (6월 5주차 목표 6/30, 4h)

> v1 에서 sim 을 제외(2026-06 결정)하면서 원안의 "PickCube 성공률 N회 측정"은 sim 결합이 필요하므로 Phase 4.5(v1.5)로 이관했다. v1 기술 코어의 종착점은 순서 4(카메라/bag 1분 dry-run)이고, 순서 5 는 그 결과를 레포에 정리하는 작업으로 축소됐다. sim 선정/PickCube 정의/제어 주기 분석(순서 3 노트)은 Phase 4.5 에서 재사용한다.

- [x] 순서 4 week11 dry-run 의 latency/throughput/안정성 수치 정리 (`week8/PRACTICE.md` 실습 체크리스트의 확정 기준)
  - [x] week11 dry-run 이 산출한 mean/p95/throughput/GPU 수치 가져오기 (순서 5 에서 새로 재지 않음). 미기록분이 있으면 dry-run 1회 재실행해 채운다
  - [x] 전체 제어 루프 2 Hz 이상(step 500 ms 이하) 충족 여부 판정 (위 "제어 주기 요구 확정" 노트 — sim 게이트가 아니라 실로봇 plausibility 하한)
  - [x] dry-run 통과 조건 확인: `0 fail` / 노드 안 죽음 / GPU < 10 GB
  - [x] 500 ms 초과로 2 Hz 밑이면 placeholder 가 아니라 실측 기반으로 기준 재조정 + 사유 메모
- [x] 결과 표 정리 — `week12/PRACTICE.md` 실습 5 의 "측정 결과" 표 형식 참고. 여기까지가 v1 레포 기록
  - [x] Mean / p95 / Throughput / GPU Memory 표 작성 (성공률 행 없음 — sim 이관)
  - [x] 한계 명시: v1 은 추론 루프 + dry-run 측정까지. task 성공률은 sim 이 결합되는 Phase 4.5 에서 측정
- [x] **8월 초 체크포인트 수행** — "진행 원칙"의 입력으로 8-12월 예산·배치 확정 (순서 6-7 잠정치 갱신, v1.5 조기 진입 여부 결정)
  - [x] 6-7월 실제 진척 속도(시간/일 추세) vs 계획 대조
  - [x] 8-12월 가용 예산 재산정 → 순서 6-7 잠정치 갱신
  - [x] v1.5(Phase 4.5) 조기 진입 여부 결정

### 노트: 8월 체크포인트 조기 수행 (2026-06-29 — 가용 시간 축소로 앞당김)

원래 8월 초 예정이나, 6/30 부터 가용 시간이 평일 2h 로 축소돼 예산을 미리 재산정한다. 정식 8월 체크포인트는 7월 실적이 쌓인 뒤 이 결론을 재확인한다(이 노트가 그 1차 입력).

- **6-7월 진척 vs 계획**: 6월(하루 5-8h)에 순서 1-4(약 56h) 완주 — 계획 페이스 충족. 순서 4(v1 기술 코어) 6/29 확보. 6/30 부터 페이스 약 3-4배 감소(주 25-40h → 8-10h).
- **8-12월 예산 재산정**: 평일 2h × 약 4-5일 = 주 약 8-10h, 월 약 30-35h(출장/피로 변수로 보수적). 남은 v1 = 순서 5(4h) + 순서 6 정독(20-30h) = 약 24-34h ≈ 약 1-1.5개월(7월, 미달 시 8월 이월).
- **순서 6-7 잠정치 갱신**: 순서 6 정독 20-30h 유지(2h 블록에 적합, wall-clock 약 3-4주). 순서 7 은 이미 v2 이관(변동 없음).
- **v1.5 조기 진입 결정 — 전면 조기 진입(9월 착수) 보류**. 사유: (1) 2h 제약으로 페이스 3-4배 감소, (2) v1 에서 sim 을 빼면서 Phase 4.5 가 sim 구축 + zero-shot baseline 측정까지 흡수해 6-8주 → 7-10주로 커짐, (3) `Studies/Phase 4.5/` 학습 자료가 week README 스캐폴드뿐이라(PRACTICE/quiz 미작성) 자료 작성 시간이 추가됨, (4) 클라우드 GPU 가용성·비용 변수. 대안: **Phase 4.5 Section 0(리스크 스파이크 — 클라우드 LoRA 1사이클 실측 + ManiSkill sim 구축 + zero-shot 성공률 baseline)만 9-10월 선행**한다. v1 과 독립적이고 리스크가 가장 크므로 분기 재평가(2026.11)를 가설이 아닌 실물 위에서 하기 위함. 본 학습(Section 1-3)은 11-12월 유지하고, Section 0 실측 결과로 일정을 재확정한다.
...
- **정식 8월 초 체크포인트 안건 (2026-07-20 등재, 입력: 7월 실가용 실적)**: (1) 조기 결론 재확인 (Section 0 2026.08 / Sections 1-3 2026.09-11), (2) 하드웨어 스파이크 실행 시기 결정 — 9월 절충안 vs 원안 10월 (8월은 GPU 종속 작업 과밀로 제외). SO-101 키트 구매 시점이 여기에 연동된다 (리드타임 3일), 구매 전 판매자 확인 4항목 처리, (3) 순서 6 정독 잠정치 갱신, (4) git filter-repo 실행 여부 확정 — 레포 비공개 유지·산출물 repo 분리(2026-07-20)로 불요 예상, 산출물 repo 신원 설정 확인으로 대체 (진행 원칙 항목), (5) **4070 PC 자택 이전 여부와 시점** (2026-07-28 조율 개시) — 성사 시 GPU 종속 작업의 8월 마감 근거가 소멸하므로 8월 배치를 재작성해야 한다. 나머지 안건보다 먼저 확정한다.
- **2026-07-28 갱신**: 자작 팔을 Koch v1.1 → **SO-101 (SO-ARM101)** 로 변경. 사유: 채용 관점에서 읽히는 것은 모터 브랜드(타깃사 액추에이터 정합)가 아니라 LeRobot/HF 파이프라인 경험이고, 그 생태계의 현행 표준이 SO-101 이다. 부수 효과 3가지 — 리드타임 40일(휴일 미포함) → 3일이라 12월 발주 마감이 소멸, 3D 프린팅 부품이 키트에 포함돼 출력 경로 확보 부담 소멸, 완제품 키트라 스파이크가 "부품 선구매" 에서 "조립 전 드라이버 선검증" 으로 재정의됨. Stage 2 의 XM430 6DOF 확장 원안은 SO-101 이 이미 6DOF 라 무효 — 확장 수단 결정을 2026.11 재평가 안건으로 이관. 상세: [Hardware-Arm.md](../../Roadmap/Hardware-Arm.md).
- **2026-07-20 갱신**: 4070 (우분투 PC) 은 **반납하지 않기로 확정**. 단 휴직 개시 (2026.09) 후에는 PC 장애 시 현장 방문 전까지 복구 불가라 GPU 종속 작업의 8월 내 완료 방침은 유지한다 (마감 근거가 "반납 전"에서 "물리 접근이 일상적인 마지막 달"로 교체). 8월 중 원격 운용 전환 점검(자동 부팅·원격 접속 경로·원격 전원 수단)을 수행한다 — 상세: remediation plan 실행 보드 #12.
