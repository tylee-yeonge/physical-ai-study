...
> **[v1 범위 조정 — 2026-06]** 이 주차(RT-2 블로그 작성)는 **v1 에서 제외하고 v2(Phase 6)로 이관**했다. v1 범위는 week1-2 의 RT-2 정독(아키텍처 이해)까지다. 정독 노트는 v2 블로그의 배경으로 재사용한다. 아래 블로그 작성 가이드는 v2 공개 단계 참조용으로 보존한다. 결정 배경: `Roadmap/Phase 4.md` "산출물 v1" 절.


> **이번 주 목표**: week 1-2 의 reading note 와 한 페이지 노트를 합쳐서 **공개 가능한 RT-2 블로그 1편** 의 초고를 마감한다. 산출물 v1 의 절반 (블로그 2편 중 1편).
...
> 본 phase 의 산출물 v1 는 "블로그 2편 + ROS2 demo" 다. 블로그 1편이 못 나오면 산출물 자체가 흔들린다. 이번 주가 본 phase 의 첫 결정타.
...
1. **블로그 1편 = 산출물 v1 의 1/3** (총: 블로그 2편 + ROS2 demo)
