...
> [goal] **이번 주 목표**: `vla_node` 라는 ROS2 ament_python 패키지를 0 부터 만들어 빌드까지 성공. callback / publisher / subscriber 의 골격 완성. 아직 inference 실제 연결 안 함 (dummy 동작).
> [time] **예상 시간**: 10시간 (패키지 setup 3h + 노드 코드 4h + 빌드/실행 3h)
> [tip] **핵심 질문**: "내 노드가 빈 동작 (dummy zero action publish) 으로 ROS2 graph 안에서 정상 동작하는가? 빌드/실행 에러 없이 colcon build 통과하는가?"
...
[O] 이전: [Week 9 - I/O 인터페이스 정리](../week9/README.md)
