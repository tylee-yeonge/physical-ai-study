...
> [!] **참고**: 이번 주는 가볍게 훑고, 이후 Phase 에서 필요할 때 돌아와서 심화 학습
...
- 위치(x, y, z): 간단하게 더하기/빼기로 업데이트 가능 → `x_new = x_old + Δx` [O]
- 회전(방향): 단순히 더하면 회전이 아닌 이상한 값이 됨 → `R_new = R_old + ΔR` [X]
...
| 2 | Python 기초 실습 | `lie_basics.py` | Lie 군/대수 기본 연산 (skew, exp, log 구현) |
| 3 | Python 심화 퀴즈 | `lie_quiz.py` | 개념 확인 퀴즈 및 ICP 구현 실습 |
...
| **Lie 대수 ξ (se(3))** | **6개** | **6개** | **제약 없음!** [O] |

> [tip] **왜 Lie 대수는 "3개 또는 6개"인가?**
> 
...
x = x + learning_rate * gradient  # [O] 일반 변수는 OK
```
...
q_new = q + Δq
# = [1.1, 0, 0, 0]   # [X] 길이가 1.1 (단위 쿼터니언이 아님)
...
#### [tool] exp: Lie 대수 → Lie 군 (Rodrigues 공식)
...
# Step 4: 회전 업데이트 (왼쪽 곱)
R_new = ΔR @ R  # [O] R_new도 항상 유효한 회전 행렬!
```
...
#### [tool] 구체적 예시: ICP (Iterative Closest Point)
...
A: Week 6은 "가볍게 훑고", Phase 3에서 다시 볼 예정입니다. 기본 개념만 알고 넘어가도 OK!
