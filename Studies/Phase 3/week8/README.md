...
깊이맵은 단일 채널이므로 컬러맵으로 색상을 매핑한다. 자주 쓰는 컬러맵:

| 컬러맵  | 색상 흐름                | 특징                              |
|---------|--------------------------|-----------------------------------|
| magma   | 어두운 보라 -> 밝은 노랑 | 논문에서 가장 많이 사용           |
...
```mermaid
flowchart TD
    A["YOLO 결과<br/>사람, x1=100, y1=50, x2=300, y2=400, conf=0.92"] --> B["깊이맵에서 해당 영역의 깊이 추출<br/>roi_depth = depth_map[50:400, 100:300]<br/>이 영역의 깊이값으로 사람까지의 거리 판단"]
    B --> C["결합 결과<br/>사람이 감지됨, 가까움!<br/>장애물 회피 시스템에 전달"]
```
...
#### Jetson에서 YOLO와 Depth를 동시에 돌릴 때

두 모델을 한 보드에서 같이 실행하면 메모리와 성능을 함께 따져야 한다(문제 3의 마지막 질문).

- **메모리**: TensorRT FP16 기준 YOLO는 약 200MB, Depth(ViT-S)는 약 350MB GPU 메모리를 쓴다. 합쳐 약 550MB로, 시스템 점유(약 2GB)까지 더해도 8GB 안에 들어온다. 다만 다른 프로세스와의 경합은 주의한다.
- **지연(latency)**: 두 모델을 **순차 실행**하면 시간이 더해진다. 예를 들어 YOLO 8ms + Depth 50ms = 58ms로 약 17 FPS다. CUDA Stream으로 병렬화해도 같은 GPU 자원을 두고 경합하므로 큰 이득은 없다.
- **프레임 교대 / 주기 실행**: 깊이는 객체 위치보다 천천히 변하므로, Depth를 매 프레임이 아니라 2-3 프레임마다 실행하거나 YOLO와 프레임을 교대로 처리하면 전체 프레임률을 유지할 수 있다.

향후 파이프라인 (Phase 3 전체):

```mermaid
flowchart TD
    A["카메라"] --> B["YOLO (객체 검출)<br/>'무엇이' 있는지"]
    A --> C["Depth Anything<br/>'얼마나 멀리' 있는지"]
    B --> D["장애물 회피 / 경로 계획"]
    C --> D
```
