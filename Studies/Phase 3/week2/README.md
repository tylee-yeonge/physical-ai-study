...
```mermaid
flowchart TD
    A["원본 이미지 1장"] -->|"Augmentation 적용"| B["여러 변형본<br/>좌우 반전본<br/>밝게 한 본<br/>어둡게 한 본<br/>노이즈 추가본<br/>약간 회전한 본 ..."]
    B --> C["모델 학습<br/>반전해도, 밝아도, 노이즈가 껴도<br/>여전히 고양이는 고양이<br/>변형에 불변인 본질적 특징을 학습"]
```
