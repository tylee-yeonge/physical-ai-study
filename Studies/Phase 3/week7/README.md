...
AMR (자율 이동 로봇) 시나리오를 보자.

- 카메라로 '사람'을 검출했다. 그런데 사람이 2m 앞에 있나, 10m 앞에 있나?
- 2m면 즉시 정지해야 하고, 10m면 감속만 하면 된다.
- 바운딩 박스만으로는 판단이 어렵다. 박스가 크면 가까운 걸까? 단지 큰 사람이라 박스가 클 수도 있다.
- 깊이 정보가 있으면 정확한 거리 판단이 가능하다.
...
- 기존 CNN의 한계: 전역 정보 부족, Receptive field 제한
- ViT의 장점: Self-Attention으로 이미지 전체를 한 번에 본다. 전역 정보와 지역 정보를 모두 활용한다.
...
```mermaid
flowchart TD
    A["Teacher 모델<br/>라벨 데이터 1.5M으로 학습<br/>(DPT-Large 기반)"] -->|"의사 깊이맵 생성"| B["비라벨 이미지 62M<br/>+ 의사 깊이맵"]
    B --> C["Student 모델<br/>(ViT-S / ViT-B / ViT-L)<br/>+ 강한 Augmentation<br/>+ 라벨 데이터 1.5M 병행 학습"]
```
...
- 이미지를 14x14 패치로 분할한다.
- 각 패치를 벡터로 변환한다 (Embedding).
- 변환된 패치들을 Transformer에 입력한다.

Self-Attention: 각 패치가 다른 모든 패치와의 관계를 학습한다. 예를 들어 하늘 패치와 바닥 패치의 관계가 깊이 단서가 된다.
