...
struct Feature {
    ...
    Feature(const cv::Point2f& pt_, int id_)
        : pt(pt_), id(id_), has_3d(false) {}
};
...
struct Frame {
    ...
    Frame(int id_, const cv::Mat& img, double ts)
        : id(id_), image(img), timestamp(ts) {}
};
...
struct MapPoint {
    ...
    MapPoint(int id_, const Eigen::Vector3d& pos)
        : id(id_), position(pos), observations(0) {}
};
...
struct VOStatistics {
    ...
    VOStatistics()
        : total_frames(0), tracked_features(0), 
          lost_tracks(0), new_landmarks(0),
