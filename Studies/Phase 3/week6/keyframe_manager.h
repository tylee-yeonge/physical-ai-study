...
{
   public:
    KeyframeSelector(int min_frames = 20, int max_frames = 30, double min_tracked_ratio = 0.5,
     ...
     */
    bool needNewKeyframe(int num_frames_since_last, int num_tracked, int total_map_points,
                         double avg_parallax);
...
class KeyframeCuller
{
   ...
   private:
     ...
     */
    bool isRedundant(const Keyframe* kf, const std::vector<MapPoint>& map_points);
};
...
class CovisibilityGraph
{
   public:
     ...
     */
    void updateCovisibility(Keyframe* kf1, Keyframe* kf2, const std::vector<MapPoint>& map_points);

    /**
     * @brief Local Keyframes 가져오기 (BA용)
     */
    std::vector<Keyframe*> getLocalKeyframes(Keyframe* curr_kf, int max_keyframes = 10);
};
