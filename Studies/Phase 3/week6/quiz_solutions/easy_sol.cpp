/**
 * Quiz Easy (정답) - Week 6: Keyframe Management
 *
 * 이 퀴즈에서 다루는 개념:
...
void problem1_keyframe_purpose()
{
    std::cout << "Q1. Keyframe을 사용하는 주요 이유 3가지는?" << std::endl;
    // ✅ 정답:
    std::cout << "   답: 맵 구축 기본 단위, Bundle Adjustment 대상, Loop Closure 참조\n" << std::endl;
}
...
void problem2_keyframe_condition()
{
...
void problem4_90_percent_rule()
{
    std::cout << "Q4. 90% rule이란?" << std::endl;
    // ✅ 정답:
    std::cout << "   설명: KF 맵 포인트의 90% 이상이 다른 3개 이상의 KF에서도 관측되면 중복으로 삭제\n" << std::endl;
}
...
int main()
{
    ...
    problem1_keyframe_purpose();
    problem2_keyframe_condition();
    problem3_culling();
    problem4_90_percent_rule();
    problem5_covisibility();
