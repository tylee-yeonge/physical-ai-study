/**
 * Quiz Medium - Week 6: Keyframe Management (정답)
 *
 * 이 퀴즈에서 다루는 개념:
...
void problem1_orb_vs_vins()
{
    ...
    // ✅ 정답:
    std::cout << "   ORB-SLAM: Vision-only → 적극 생성 + 90% Rule Culling (크기 가변)" << std::endl;
    std::cout << "   VINS: Vision+IMU → 보수적 생성 + Sliding Window (크기 고정)\n" << std::endl;
}
...
void problem2_culling_timing()
{
    ...
    // ✅ 정답: b) 새 Keyframe 추가 시
    std::cout << "   가장 좋은 답: b) 새 Keyframe 추가 시\n" << std::endl;
}
...
void problem3_covisibility()
{
    ...
    // ✅ 정답: 공유 맵 포인트가 많은 KF → 비슷한 장면 관측
    std::cout << "   설명: 공유 맵 포인트가 많아 비슷한 장면을 관측하는 KF" << std::endl;
    std::cout << "   → Local BA에서 함께 최적화, Loop Closure 후보\n" << std::endl;
}
...
void problem4_first_keyframe()
{
    ...
    // ✅ 정답: 좌표계 원점 + Gauge freedom 고정
    std::cout << "   1) 좌표계 원점: 모든 포즈/맵 포인트의 기준점" << std::endl;
    std::cout << "   2) Gauge freedom 고정: BA의 7 DoF 자유도 제거" << std::endl;
    std::cout << "   3) 초기 맵의 anchor: 전체 맵의 기반\n" << std::endl;
}
