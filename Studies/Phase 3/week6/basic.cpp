...
bool KeyframeCuller::isRedundant(
    ...
    for (int mp_idx : kf->map_point_indices) {
        if (mp_idx < 0 || mp_idx >= map_points.size()) continue;
...
void CovisibilityGraph::updateCovisibility(
    ...
    if (num_shared >= 15) {  // 임계값
        kf1->addCovisibility(kf2, num_shared);
        kf2->addCovisibility(kf1, num_shared);
    }
}

std::vector<Keyframe*> CovisibilityGraph::getLocalKeyframes(
    Keyframe* curr_kf,
    int max_keyframes
) {
...
void demoKeyframeManagement() {
    ...
    for (int frame = 0; frame < 200; frame++) {
        ...
        bool need_kf = selector.needNewKeyframe(
            frames_since_last, num_tracked, total_map_points, avg_parallax);
