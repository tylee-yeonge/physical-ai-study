...
```mermaid
flowchart TD
    A["ONNX 모델 입력"] --> B["각 레이어에 대해 커널 후보 나열"]
    B --> K1["커널 후보 A: im2col + GEMM"]
    B --> K2["커널 후보 B: Winograd"]
    B --> K3["커널 후보 C: FFT"]
    B --> K4["커널 후보 D: Direct convolution"]
    K1 --> M["각 후보를 실제 GPU에서<br/>실행하여 시간 측정"]
    K2 --> M
    K3 --> M
    K4 --> M
    M --> S["가장 빠른 커널 선택"]
    S --> E["TensorRT Engine (.trt) 저장"]
```

빌드 시간은 5-30분이지만 한 번만 하면 된다. 다만 GPU마다 다른 엔진이 생성되어 이식은 불가능하다.
...
```mermaid
flowchart LR
    A["카메라 입력<br/>(OpenCV)"] --> B["전처리<br/>(GPU에서)"]
    B --> C["TensorRT<br/>추론"]
    C --> D["NMS<br/>후처리"]
    D --> E["시각화/출력<br/>(OpenCV)"]
```
...
YOLO 출력은 8400개의 후보 박스다. 같은 객체(예: 사람) 하나에 여러 박스가 겹쳐서 나온다.
...
```mermaid
flowchart LR
    A["YOLO 출력<br/>8400개 후보 박스"] --> B["같은 객체에<br/>box1, box2, box3 겹침"]
    B -->|"NMS 적용"| C["box1만 남김<br/>(가장 높은 confidence)"]
```
