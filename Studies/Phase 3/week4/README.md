...
**마지막 10 에폭에서 Mosaic을 끄는 이유**가 중요하다. Mosaic으로 합성된 이미지는 실제 입력 이미지와 분포가 다르다. 학습 막바지에는 실제 이미지로 마무리해야 모델이 실제 추론 환경에 적응해 최종 성능이 안정된다.
...
위 5단계는 "변수를 통제하며 실험한다"는 사고를 익히기 위한 **학습용 골격**이다. 실무에서는 그대로 돌리지 않는다. 한계를 알고 써야 한다.

- **데이터 품질이 하이퍼파라미터보다 압도적으로 중요하다.** `lr0`를 0.001 vs 0.01로 바꿔 얻는 mAP 차이보다, 라벨링 오류를 고치고 데이터를 더 넣는 게 훨씬 크다. YOLO 기본값은 COCO에서 이미 튜닝된 값이라 어지간하면 잘 동작한다. 그래서 실무자는 `lr0` 같은 값을 거의 기본값 그대로 두고 데이터에 시간을 쓴다.
- **하이퍼파라미터는 서로 상호작용한다.** 모델을 `yolo11n`에서 `yolo11m`으로 키우면 최적 `lr0`도 `batch`도 달라진다. 그래서 "3단계에서 찾은 최적 lr"이 5단계에서 `imgsz`를 바꾸면 더는 최적이 아니다. 즉 순차 단계는 진짜 최적값이 아니라 **근사**일 뿐이다.
- **풀 스윕은 비현실적이다.** 학습률 4개 x 모델 3개 x augmentation 2개 x 이미지 크기 3개를 다 곱하면 72번 학습이다. 아무도 이렇게 안 돌린다. 진짜로 최적을 찾으려면 grid/random search나 Optuna 같은 자동 탐색을 쓴다. YOLO에도 `model.tune()`이 있다.
- **실제로는 증상을 보고 처방한다.** baseline을 돌려보고 명백히 부족한 것 하나만 건드린다. 과적합이면 augmentation을 강화하고, 작은 물체를 못 잡으면 `imgsz`를 키우고, GPU/시간 여유가 있으면 모델 크기를 한 단계 올려본다.

**학습 단계에서의 목적**은 이 5단계를 직접 몇 번 돌려보며 "lr을 키우니 loss가 발산하더라", "imgsz를 키우니 작은 물체 mAP가 오르더라" 같은 감을 잡는 것이다. 그 감이 생기면 실무에서는 이 5단계를 다 돌지 않는다.
...
**`best.pt`와 `last.pt`의 차이**가 중요하다. `best.pt`는 학습 도중 검증 mAP가 가장 높았던 에폭의 가중치이고, `last.pt`는 마지막 에폭의 가중치다. 마지막 에폭이 항상 최고는 아니다 - 과적합으로 오히려 나빠졌을 수 있다. 그래서 **배포에는 항상 `best.pt`**를 쓰고, `last.pt`는 학습을 이어서(resume) 할 때 쓴다.
...
| 실제 \ 예측 | person | car | bicycle | background |
|------------|:------:|:---:|:-------:|:----------:|
| person     | 85     | 2   | 1       | 12         |
| car        | 3      | 78  | 0       | 19         |
| bicycle    | 1      | 0   | 65      | 34         |
...
`quiz_easy.py` 문제 4가 학습 곡선(train_loss는 계속 내려가는데 val_mAP가 떨어지는 상황)을 주고 과적합 여부와 해결책을 묻는다.
