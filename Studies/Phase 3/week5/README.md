...
FP16이 가성비 최고 - 정확도 손실 거의 없이 2배 빨라짐.
...
#### 3단계 자세히: 스케일과 제로 포인트 결정

3단계가 캘리브레이션의 핵심이다. 2단계에서 측정한 실수 범위를 받아, FP32 실수를 INT8 정수로 바꾸는 변환 규칙 두 개를 정한다.

양자화는 연속적인 FP32 실수를 256칸(INT8: -128-127 또는 UINT8: 0-255)의 정수로 매핑하는 작업이다. 이 매핑을 정의하는 파라미터가 **스케일**(scale)과 **제로 포인트**(zero point)다.

```
실수값 r = scale * (정수값 q - zero_point)
정수값 q = round(r / scale) + zero_point
```

| 파라미터 | 의미 |
|----------|------|
| scale (S) | 정수 한 칸이 표현하는 실수 간격. 작을수록 촘촘하지만 표현 범위가 좁아진다 |
| zero_point (Z) | 실수 0.0이 매핑되는 정수 위치. 패딩/ReLU 출력의 0을 오차 없이 표현하기 위함 |

2단계에서 측정한 범위 `[r_min, r_max]`로부터 S와 Z를 계산한다.

```
비대칭(Asymmetric, UINT8 0-255):
  S = (r_max - r_min) / 255
  Z = round(0 - r_min / S)

대칭(Symmetric, INT8 -128-127, 가중치용):
  S = max(|r_min|, |r_max|) / 127
  Z = 0   (제로 포인트 고정 -> 곱셈 단순화)
```

예를 들어 어떤 레이어 출력 범위가 `[-2.0, 6.0]`이면 비대칭 방식으로 `S = 8.0/255 = 0.0314`, `Z = round(2.0/0.0314) = 64`가 된다. 즉 실수 0.0은 정수 64로, -2.0은 0으로, 6.0은 255로 매핑된다.

#### "최적"이 중요한 이유: 범위를 어디서 자를까

여기서 "최적"이 핵심이다. 측정된 min/max를 그대로 쓰면 안 되는 경우가 많기 때문이다. 분포에 극단적 이상치(outlier)가 하나라도 있으면, 그 값까지 범위에 넣느라 scale이 커지고 정작 값이 몰려 있는 구간의 해상도가 망가진다.

```
값 분포: [-1, ... 대부분 여기 몰림 ..., 1]  + outlier 50.0 한 개

min/max 그대로:   S = 51/255 = 0.20   -> 대부분 값이 몇 칸에 뭉개짐 (손실 심각)
[-1, 1]로 클리핑: S = 2/255 = 0.0078  -> 대부분 값을 촘촘하게, outlier만 포화
```

그래서 "최적의 스케일 결정"은 **어디서 범위를 잘라낼지**(clipping threshold)를 고르는 문제가 된다.

| 방법 | 원리 |
|------|------|
| Max (단순) | 측정된 절대 최대값 그대로 사용. 이상치에 취약 |
| Percentile | 상위 99.9% 등 백분위로 잘라 이상치 제거 |
| Entropy (KL divergence) | 양자화 전후 분포의 정보 손실을 최소화하는 임계값 탐색. TensorRT 기본 |
| MSE | 양자화 오차의 제곱합을 최소화하는 임계값 탐색 |

TensorRT의 IInt8EntropyCalibrator는 여러 후보 임계값으로 잘라본 뒤, 원본 FP32 분포와 양자화된 분포 사이의 KL divergence가 가장 작은 지점을 "최적"으로 선택한다. 이 S, Z가 정해져야 비로소 4단계에서 실제 정수 변환(`q = round(r/S) + Z`)이 가능하다.
...
조건: 모델 YOLO11n / 입력 640 x 640 x 3 / GPU NVIDIA (CUDA) / 100회 평균
...
ONNX Runtime + TensorRT EP가 가장 빠르다.
