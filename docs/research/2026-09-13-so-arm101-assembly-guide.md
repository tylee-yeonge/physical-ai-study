...
| 카메라 | 정면 거치 모듈 1개 기본. 손목 마운트 없음 | §7 말미 | 스파이크 D8 에서 보유 ELP 장착 여부 확인 (roadmap §3) |
| 호스트 | Ubuntu 22.04 + RTX 4070 | §3.1 | macOS 절(§3.2)은 참고용 |
...
| Stage 1 W5 (물리 e-stop) | §7 비상 정지 — DC 차단은 토크 해제(낙하), USB 차단은 현 위치 유지 |
...
동작이 확인되면 카메라 추가(`lerobot-find-cameras opencv`, USB 허브 없이 직결, MJPG 권장) 후 `lerobot-record` 로 넘어간다.
