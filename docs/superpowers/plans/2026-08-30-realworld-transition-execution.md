...
| (선택) 코스 BoM | vial·랙 등 NVIDIA SO-101 코스 작업대 자재 | 코스 Phase A/B 는 재평가 #1 안건 — 기본 미포함 (master roadmap §4) |
...
- [x] **판정 기록 (2026-09-20, 1회만)**: `Studies/Hardware-Arm/spike/week2/RESULT.md` §5 에 결과 행 명시 — **must 4개 통과** → Stage 1 본 빌드 2026.10 개시, v2.5 2026.11 개시. 예정일 09-21 의 전날, must 4개의 증거가 모두 확보된 시점에 판정
...
> **라이트 모드 (2026-08-31 확정 — v1.5 급 검증 밀도 반복 금지)**: 문서 상한 = vla-lab 글 1편 + Measurements 디렉토리 1개. 검증은 v1.5 자산 (하네스 검증 논리·사전 등록 통계) 재사용으로 한정하고 새 검증 체계를 만들지 않는다. **N 은 "N≥20" 이 아니라 v1.5 방식대로 해상도 역산으로 재결정한다** — N=20 의 Wilson 반폭은 ±20%p 로 신호가 안 잡힌다. 실기 측정 설계 (물체 초기 배치 마커·리셋 프로토콜·세션 순서) 를 측정 전에 1페이지로 고정한다.
...
> **⚠ 지위 이양 (2026-08-31)**: 본 절의 마스터 보드 기능은 [master roadmap](2026-08-31-master-roadmap.md) 으로 이양됐다 — **체크는 그쪽에서만 한다.** 본 문서는 절차·통과 기준의 원본 (§4-§8) 으로 유지된다.
...
- [x] **v1.5 마감 문서를 vla-lab 에 발행** — 완료 (2026-08-31): 본문 + measurements 3건 전문 + reproduce 절차 (https://github.com/tylee-yeonge/vla-lab/tree/main/v1.5-openvla-lora-sim). LinkedIn 링크 공유는 probe 2단 일정에 맞춰 사용자 수행
