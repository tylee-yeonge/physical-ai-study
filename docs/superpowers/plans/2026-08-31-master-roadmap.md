...
| 2027.03-04 | **복직** — 지원 준비 마감 (지원 개시는 05-06, 확정 2026-09-01) | Phase 5 (규모는 재평가 결정) | — |
| 2027.05-06 | **실지원 개시 (첫 웨이브)** + Stage 2 | 디버깅 도구 곁다리 | 첫 서류·코테 / 재평가 #2 (05) |
| 2027.05-07 | Phase 6 → v2 | — | v2 공개 07 |
| 2027.07-09 | 첫 면접·오퍼 구간 — **v2 (07 공개) 가 면접에 합류** | Phase 7 착수 (08) | v3 진행 |
| 2027.10-11 | Phase 7 → v3 | 2차 웨이브 판단 | v3 공개 |
...
- [x] D8 카메라 세팅 (전체 뷰 ELP + 손목) + 테스트 녹화 — 절차: [week2_guide](../../../Studies/Hardware-Arm/spike/week2/week2_guide.md) §1. 2026-09-16 완료: 고정 경로 `/dev/so101_cam_overview`(ELP) · `/dev/so101_cam_wrist`(손목) 은 `so101-attach` 가 생성. 설정은 front 1280x480 MJPG **60 fps** (ELP 는 25/60 만 지원, 30 은 오류) + wrist 1280x720 MJPG 30 fps. 테스트 녹화 2 ep · 1198 frames · 29.92 Hz · 예산 초과 0 — 증거와 막힌 지점은 `RESULT.md` §2 · §4
  - [x] 수령 확인 ③ — 기본 정면 거치 모듈에 ELP 1/4 나사 장착 **불가**. 임시 고정으로 우회, Week 2 내내 구도 유지 — `RESULT.md` §4 #5
- [x] D9 task 정의 (예: 큐브→트레이) → `lerobot-record` 10ep → HF Hub(private) → **must 2** (증거: repo id. 부수 실측: **에피소드당 소요 시간** — v2.5 N 역산의 입력) — `--robot.id`/`--teleop.id` 는 캘리브레이션과 동일 값, 캘리브 경로 환경변수가 그 셸에 잡혀 있는지 먼저 확인 (없으면 기본 경로로 조용히 갈라진다 — 가이드 §6.3). `--dataset.no_stamp=true` 필수 (없으면 repo id 에 타임스탬프가 붙어 D10 참조가 어긋남), `--display_data=false`, VS Code 터미널(TTY) 에서 실행 (키 조작) — 절차: week2_guide §2. 2026-09-20 완료: `tylee-yeonge/so101-spike-pick-cube` (private, 10 ep · 8980 frames · 29.91 Hz · 예산 초과 0). 에피소드당 소요 61.4 s (순수 녹화 29.9 s + 오버헤드 31.5 s), 실제 시범 길이 평균 20.3 s. 완결 9개 + 30초 상한에 걸린 미완 1개 — 내용 검증 · 알려진 결함 · 막힌 지점은 `RESULT.md` §1 행 2 · §3 · §4 #9-10
- [x] D10 `lerobot/smolvla_base` zero-shot 1회 → **must 3** (증거: 영상+로그. 막힘: 카메라 키 이름 불일치) — 실행은 `lerobot-rollout` (lerobot 0.6.2 의 `lerobot-record` 는 녹화 전용), 카메라 이름은 `camera1`(손목) · `camera2`(ELP), 첫 실행 `--robot.max_relative_target=3` (틱당 도). 예상 동작은 모든 관절이 0도 근처로 모이는 것 (정규화 통계 미적용 — `RESULT.md` §4 #8) — 절차: week2_guide §3. 2026-09-20 완료: `base` 1회 + `episodic` 1회 (영상 · 관절값을 데이터셋으로 남기는 실행), 둘 다 `Rollout finished` 까지 오류 없음. 팔이 리더 없이 스스로 일어남 (`shoulder_lift` 72도 · `elbow_flex` 57도 이동), 정책 출력은 6관절 모두 -2.3 .. +4.9 (예상대로 0 근처), 제어 루프 28.5 Hz. `elbow_flex` 는 `max_relative_target=3` 아래서 +42도에 정지 — 증거 · 수치 · 원인 가설은 `RESULT.md` §1 행 3 · §4 #11
...
- [x] D12-14 `RESULT.md` 기입 (증거 4건 + 소요 + 막힌 지점 — 막힌 지점은 가이드 §9 함정 표와 대조해 새로 만난 것만) → **판정 (1회)** 기록. 2026-09-20 판정 (예정일 09-21 의 전날, must 4개의 증거가 모두 확보된 시점): **must 4개 통과** → Stage 1 본 빌드 2026.10 개시, v2.5 2026.11 개시. 막힌 지점 11개 · 분기 재평가 #1 입력 요약은 `RESULT.md` §4 · §5

### 09 하순 — 집계 + JD 착수

- [x] 9월 실적 집계 — 방법: 달력·커밋 로그로 주별 실투입 h 표 1개 (계획 대비. 재평가 #1 입력)
  - [x] 09-21 이후 두 행 기입 + 09-01 - 09-11 공백을 달력으로 확정 (월말)

  진행 상황 (2026-09-21): 09-20 까지 집계. 실투입은 기록에서 역산한 추정치다 — 커밋·Claude 세션 기록의 첫-끝 시각 구간 합 + 세션 밖 하드웨어 작업 (`spike/week2/RESULT.md` §3).

  | 주 | 계획 (roadmap 구간) | 실투입 h (추정) | 근거 | 비고 |
  |---|---|---|---|---|
  | 09-01 - 09-06 | 구매 주간 — 발주·수령 확인 (경량 3건은 스킵 처리) | 0 | 커밋 0 · 세션 기록 0. 발주 09-01 | 0h 인지 기록만 없는 것인지 구분 불가 |
  | 09-07 - 09-13 | 스파이크 Week 1 (5일 + 버퍼 2일) | 6.9 | 09-12 밤 - 09-13 새벽: 조립 3.5 (조립 가이드 참고치 — 실측 아님) + 셋업·캘리브·teleop 1.5 (`RESULT.md` §3) + 조립 가이드 작성 0.3 (01:27-01:45). 09-13 저녁 1.6 (19:08-20:44, Week 2 가이드·latency 스크립트) | 09-07 - 09-11 기록 없음. 하룻밤 + 저녁 한 번에 몰림 |
  | 09-14 - 09-20 | 스파이크 Week 2 (4일 + 판정) | 9.0 | 09-16 1.9 (22:55-00:48, D8) · 09-17 0.8 · 09-18 0.1 · 09-19 4.1 (11:51-12:15 / 17:11-18:36 / 22:50-01:08 — D11·must 1·D10 사전 검증) · 09-20 2.1 (22:00-00:04 — D9·D10·판정, 끝 0.5 는 JD 조사) | 09-15 23:06 venv 설치는 시각 1점뿐이라 합계에서 제외 |
  | 09-21 - 09-27 | 집계 + JD 착수 (메인은 휴식·버퍼) | | | |
  | 09-28 - 09-30 | 집계 + JD 착수 (계속) | | | |
  | 합계 (09-20 까지) | 월 30-35h 가정 → 20일 비례 20-23h | 15.9 | | 가정 대비 약 70-80% |

  - 산정의 오차 방향: 구간 안의 육아 중단이 빠지지 않아 과대, 프롬프트 없이 하드웨어만 만진 시간이 빠져 과소. 10분 간격 활동 블록만 합친 하한은 09-13 이후 7.7h (같은 기간 구간 합 10.9h)
  - 두 가지 읽기: 20일 전체 기준 월 환산 약 24h (가정 미달) / 작업한 9일 (09-12 - 09-20) 기준 월 환산 약 53h (가정 초과). 10-12월의 기준이 어느 쪽인지는 09-01 - 09-11 공백의 원인 (수령 대기·이전·적응기) 이 반복되는 성격인지에 달려 있다 — 재평가 #1 에서 판정
  - 세션 패턴: 평일은 22시 이후 1-2h, 주말에 낮 블록이 붙는다. 팔을 쓰는 작업은 전부 22시 이후였다. Stage 1 의 W 단위 항목은 1-2h 세션으로 끊기는 크기여야 한다
- [x] JD 정독 착수 — 회사별로 [JD 원문 저장 → 자격/우대 추출 → 산출물 매핑 메모] 1세트씩 ([JD 조사](../../research/2026-08-31-kr-physical-ai-jd-survey.md) §5 정독 포인트):
  - [x] 로보티즈 (시스템 개발직군)  
  - [x] RLWRLD (Robotics S/W)  
  - [x] 홀리데이로보틱스  
  - [x] 에이로봇
  - [x] 레인보우로보틱스  
  - [x] 두산로보틱스  
  - [x] 현대차 로보틱스랩 (9월 공고 열리면 즉시)  
  - [x] 네이버랩스
...
- [ ] **W8** 1분 영상 (teleop + 정책 실행 + 소프트웨어 정지 시연) + `stage1/README.md` 체크리스트 대조
- 병행 (≤2): - [ ] JD 격차 매핑 1페이지 마감 (10월 — 8개사 자격요건 vs v1/v1.5/v2.5/Stage 1. 입력: `.private/jd/2026-09-jd-reading.md` §3-§4. 원문 확인 결과 매핑의 기준 공고는 로보티즈 모방학습·네비게이션 엔지니어와 RLWRLD Applied Research Engineer (Robotics)·Robotics Deployment Engineer 다 — 위 09 하순 체크박스의 "시스템 개발직군"·"Robotics S/W" 는 각각 펌웨어·실시간 제어 직무였다)  - [ ] probe 택1 (커피챗 1-2건 **또는** 로보티즈 AI 사피엔스 코드 정독+이슈·기여 1건)
...
- [ ] **코테·면접 준비 개시 (확정 2026-09-01, 주 2h 경량 병행 — 2027.02 까지 지속)** — 격주: C++ 코어 문제 풀이 (RAII·스마트 포인터·동시성 위주) / 월 1-2개: 코어 서사 언어화 (QoS·discovery·대역폭·배포·장애복구 중 하나씩, 기존 5년 장애 경험과 연결). 2027.02말 C++·DDS 집중 2-3주 (재평가 결정 시) 의 기초가 된다
...
- [x] ~~2027 중반 이직 목표 확정 여부~~ → **결정 (2026-09-01)**: 지원 개시 기본선 = **복직 (2027.03) 후 2-3개월 = 2027.05-06** (휴직 2027.02 까지 — 휴직 중·복직 직후 지원 안 함). ① 조기 지원 (2027.01) **폐기** ② 패키징 전진 **불요** (2027.02 원안 유지, 오히려 여유 확보) ③ **코테·면접 준비 분산 확정** — 2026.12 부터 주 2h (§3 12월). 성사 시점 기대값은 2027 하반기 레인지. 영향 문서: 2027 실행 가이드·positioning spec §6.2 (재평가 #1 재작성 시 이 기본선 반영), README 부록 D 실지원 안건은 기본선 재확인으로 축소
