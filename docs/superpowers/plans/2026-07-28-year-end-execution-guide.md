...
| 무산 또는 지연 | 현 계획 유지 + **Stage 1 의 Isaac Sim 항목이 위험해진다.** Isaac Sim GUI 는 WebRTC 가 UDP 를 요구하는데 RunPod 은 UDP 미지원이라 클라우드 이관이 불가하고, 원격 데스크톱 경유는 GPU 렌더 성능상 권장되지 않는다 (Hardware-Arm.md 실행 머신 절) |
