...
| 항목 | 내용 | 추정 |
|---|---|---|
| Block 2 마감 | [findings.md](../../../Measurements/openvla-rtx4070-int4/findings.md) §4.2 Step 5 (fp16 대비 int4 속도 방향 예측 → Table 2 대조) + 완료 판정 표 4행 → 커밋 (remediation Task 2-1 Step 3) | 1-1.5h |
...
7월 잔여 3-4일에 Section 0 을 착수하지 않는 이유: Section 0 은 추정 30-40h 라 6-8h 로는 5분의 1도 진행되지 않고 중단된다. 반면 Block 2 는 5분의 4가 완료된 상태라 중단 후 재진입 비용이 가장 큰 항목이고, 남은 분량이 잔여 기간에 맞는다. Section 0 의 실제 마감은 8월 말이 아니라 **Section 1 착수 (9월 중순) 전에 zero-shot baseline 확보**이므로 (§3.2 3층), 8월 2주차 착수로도 일정이 성립한다.
...
여기에 이 문서가 추가로 올리는 안건이 둘 있다.

**(5) §3.2 의 3층 분리를 채택할지.** 채택하면 8월 미완 항목이 "이월 예정" 으로 명시되고, 채택하지 않으면 8월 말에 무엇이 미완인지가 우연히 결정된다.

**(6) 4070 PC 자택 이전 여부와 시점** (2026-07-28 조율 개시). 이 결론이 8월 계획의 형태를 바꾼다:
...
안건 (5) 의 결론이 (6) 에 종속되므로 (6) 을 먼저 확정한다.
...
| 이 문서의 항목 | 원본 위치 |
|---|---|
| 실행 보드 #1-#15 | remediation plan §실행 보드 |
