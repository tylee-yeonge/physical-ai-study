...
> **⚠ 보드 동결 (2026-08-31)**: 이 보드의 체크 기능은 [master roadmap](2026-08-31-master-roadmap.md) 으로 이양됐다 — **일상 체크는 그쪽에서만 한다.** 미완 항목의 처분 (완료/취소/이월/대체) 은 master roadmap §4 스킵·연기 대장이 원본이다. 본 보드는 기록 보존용으로 동결.
...
- [x] 15. **Phase 4.5 Sections 1-3** — 완료 (2026-08-31, v1.5 vla-lab 발행) — 자료 `Studies/Phase 4.5/week1` - `week6`. week3 -> week4 -> week5 -> week6 순서 고정 (각 week 이 앞 week 산출물을 입력으로 받는다)
...
- [x] 6. **Section 0 후반: Docker 컨테이너화 → RunPod 기동·재현 확인 + LoRA 1사이클 실측 + fine-tuned 4-bit 의 4070 안착 확인** — 완료 (2026-08: LoRA 94분/$1.18, 4bit 안착 4.38GB — `Measurements/openvla-lora-runpod`·`openvla-lora-eval`)
