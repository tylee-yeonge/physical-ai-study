...
| 구간 | 계획 투입 | 내용 |
|---|---|---|
| 6월 2주차 | 6h | Step 0: SETUP + 4-bit 로드 + latency/VRAM 실측 |
| 6월 2-3주차 | 4h | 표적 skim (OpenVLA §3-4 + 모델 카드 + 공식 repo README: action 표현 / unnorm_key / 입력 형식 / embodiment 가정) |
| 6월 2-3주차 | 12h | sim 환경 선정 + embodiment 정합 + 성공 task 1종 정의 (→ latency 판정 수치 확정) |
| 6월 3주 - 7월 중순 | 34h | week8-12 실습 압축 (VLAInference → ROS2 노드 → sim 루프) |
| 7월 중순 - 말 | 22h | 성공률 N회 측정 + 결과 정리 |
| **6-7월 합계** | **78h** | 계획선 정합. 최대 가용(96h)과의 차이 약 18h 는 버퍼로, 막히는 구간(실습/측정)에 흡수 |
| 8-9월 (주 6-8h) | 50-70h | RT-2/OpenVLA 정독 + 블로그 2편 + 패키징 + 영상 + v1 공개 |
