...
> **상태**: **supporting 증거로 공개** — 대표작 아님. YOLO+Depth 조합은 AMR ROS 5년차 기준 commodity 이므로 헤드라인에 올리지 않고, TensorRT/양자화 배포 + ROS2 통합의 보조 증거로만 공개한다 (vla-lab/LinkedIn 어필 안 함). v1 공개 전까지 README 계층에서 하위 배치 (헤드라인 오독 방지). 학습/통합 가치(양자화 배포, 추론 노드 스캐폴드)는 VLA v1 에 흡수.
...
> **fallback 예외**: 부록 E(AMR/AV Perception 착지) 진입 시, 이 supporting 로그를 vla-lab 문서 1편으로 승격 공개해 공백기 산출물로 활용한다.
...
> week8 통합 노드가 본 Phase 의 핵심 산출이자 VLA v1 wrapper 의 스캐폴드다. 빌드/실행 후 repo 에 supporting 로그로 커밋한다(vla-lab 공개 안 함).
...
- [ ] repo supporting 로그 커밋 + 짧은 노트 (vla-lab/LinkedIn 공개 안 함)
...
- **하드웨어 스파이크 (2026.09, 2주)** — SO-101 (Feetech) LeRobot 네이티브 검증 (산출물 아님. 정의: [Hardware-Arm.md](Hardware-Arm.md))
