...
> [time] **기간**: 약 3개월 (2027.02~05, F안 — v2 의 6개월에서 압축)
> [goal] **목표**: 자작 팔 (Hardware-Arm Stage 2) 의 URDF 를 Isaac Sim 에 임포트 → 디지털 트윈 + Sim/Real gap 측정 인프라 구축
> [code] **언어**: **Python** (Isaac Sim API) + **ROS2**
> [tool] **하드웨어**: Ubuntu PC (RTX 4070) + 자작 팔 (Hardware-Arm Stage 2, 6DOF)
...
**핵심 메시지**:
- 본 Phase 는 *Sim only 산출물* 을 만들지 않는다 (#3 폐기, F안). Sim 환경 자체는 Phase 7 의 *Real-to-Sim-to-Real* 산출물 #4 의 한 축으로 통합.
...
## [list] Section 8.0: 시작 전 (Phase 5 종료 후)
...
## [list] Section 8.1: Isaac Sim 환경 셋업 (3주)
...
## [list] Section 8.2: URDF 임포트 + 디지털 트윈 (4주)
...
## [list] Section 8.3: Sim/Real gap 측정 인프라 (5주)
