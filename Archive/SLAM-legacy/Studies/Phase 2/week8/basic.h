...
class OpticalFlowBasic
{
   public:
     ...
     * @return 추적 성공률
     */
    static double lucasKanade(const cv::Mat& prev_img, const cv::Mat& curr_img,
     ...
     * @param flow 광학 흐름 (출력)
     */
    static void farneback(const cv::Mat& prev_img, const cv::Mat& curr_img, cv::Mat& flow);
     ...
     * @param step 화살표 간격
     */
    static void visualizeFlow(const cv::Mat& flow, cv::Mat& output, int step = 16);
     ...
     * @param output 출력 이미지
     */
    static void visualizeSparseFlow(const cv::Mat& img, const std::vector<cv::Point2f>& prev_pts,
