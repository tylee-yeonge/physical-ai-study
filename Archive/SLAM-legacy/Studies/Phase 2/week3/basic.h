...
class FeatureDetectionBasic
{
   public:
     ...
     * @return 검출 시간 (ms)
     */
    static double detectFAST(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints,
                             int threshold = 20, bool nonmaxSuppression = true);
     ...
     * @return 검출 시간 (ms)
     */
    static double detectORB(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints,
                            cv::Mat& descriptors, int nfeatures = 500);
     ...
     * @param title 윈도우 제목
     */
    static void visualizeKeypoints(const cv::Mat& image, const std::vector<cv::KeyPoint>& keypoints,
                                   cv::Mat& output, const std::string& title = "Keypoints");
    ...
    static cv::Mat analyzeDistribution(const std::vector<cv::KeyPoint>& keypoints,
                                       cv::Size imageSize, int gridSize = 8);
