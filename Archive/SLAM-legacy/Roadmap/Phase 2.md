...
- [x] 3D → 2D 투영 과정 이해
- [x] 초점 거리 (Focal Length) 개념
- [x] 주점 (Principal Point) 개념
...
- [x] 카메라 내부 행렬 K 구조 이해
  ...
  K = [fx 0 cx]
      ...
      [0 0 1]
  ```
- [x] fx, fy가 왜 다를 수 있는지 (픽셀 비정방형)
- [x] cx, cy가 이미지 중심이 아닐 수 있는 이유


#### 외부 파라미터 (Extrinsic)
- [x] 월드 좌표 → 카메라 좌표 변환
- [x] 회전 행렬 R과 평행이동 벡터 t
- [x] [R|t] 행렬의 의미


#### SLAM에서 어디에 쓰이나?
- [x] VINS config 파일의 카메라 파라미터가 이것
...
> **하드웨어**: Jetson Orin Nano + ELP Stereo (캘리브레이션 완료 필요)
> **실습 시간**: 18-24시간 (3주)
...
> **하드웨어**: Jetson Orin Nano + ELP Stereo
> **실습 시간**: 6-8시간
