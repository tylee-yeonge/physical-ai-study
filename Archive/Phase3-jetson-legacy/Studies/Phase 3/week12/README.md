...
> [goal] **이번 주 목표**: 전체 파이프라인의 성능을 최적화하고, Phase 3 학습 내용을 포트폴리오로 정리하기
> [time] **예상 시간**: 12시간
> [tip] **핵심 질문**: "실시간 2D Perception 시스템을 포트폴리오로 어떻게 보여줄 수 있는가?"
...
필요한 선행 지식:
  [O] PyTorch (Phase 3에서 완료)
  [O] 3D 좌표 변환 (Phase 3에서 완료)
  [O] TensorRT 배포 (Phase 3에서 완료)
  [pin] PCL (Point Cloud Library) 기초
  [pin] Open3D 라이브러리
```
